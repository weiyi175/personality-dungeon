...
# 2D expected grid around B-center best candidate.
#
# Default grid:
#   a in {0.76, 0.80, 0.84}
#   b in {0.67, 0.69, 0.71}
...
J_MAX="${J_MAX:-12}"
RUN_TAG="${RUN_TAG:-hopf_grid2d_expected_b_center_$(date +%Y%m%d_%H%M%S)}"
...
ROUNDS="${ROUNDS:-8000}"
A_GRID="${A_GRID:-0.76 0.80 0.84}"
B_GRID="${B_GRID:-0.67 0.69 0.71}"
...
echo "=== Launch 2D expected grid around B-center ==="
for aval in "${A_ARR[@]}"; do
  atag="$(printf '%s' "$aval" | tr '.' 'p')"
  for bval in "${B_ARR[@]}"; do
    btag="$(printf '%s' "$bval" | tr '.' 'p')"
    for seed in "${SEED_ARR[@]}"; do
      out="$OUT_DIR/hopf_gridB_a${atag}_b${btag}_seed${seed}_r${ROUNDS}.csv"
