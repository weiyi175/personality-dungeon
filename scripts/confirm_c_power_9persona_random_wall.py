#!/usr/bin/env python3
"""Confirm whether a 9-persona random wall exists for W4 C-power setting.
...
- evolution_mode=sampled, popularity_mode=sampled
...
def main() -> int:
    p = argparse.ArgumentParser(description="Confirm 9-persona random wall for C-power")
    ...
    out_dir = ROOT / "outputs" / "c_power_9persona_wall_check"
    ...
    i = 0
    print("C-power 9-persona random wall confirmation")
    ...
    summary = {
        "experiment": "C-power 9-persona random wall check",
        "gamma": float(args.gamma),
        "power": float(args.power),
        "rounds": int(args.rounds),
        "players": int(args.players),
    ...
    md_lines = [
        "# C-power 9-persona Random Wall Check",
        "",
        f"- gamma: {args.gamma}",
        f"- power: {args.power}",
