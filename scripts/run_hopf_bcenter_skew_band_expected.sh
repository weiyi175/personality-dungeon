...
# Expected-mode skew-band search around the B-center candidate.
#
# This is intentionally not a symmetric Cartesian grid.
# It runs a diagonal spine plus an upper-side extension, so the search spends
# more budget on the higher-b side instead of mirroring both flanks.
#
# Default pairs:
#   spine        : (0.76,0.67) (0.78,0.68) (0.80,0.69) (0.82,0.70) (0.84,0.71)
#   upper flank  : (0.80,0.70) (0.82,0.71) (0.84,0.72) (0.86,0.73)
#   inner bridge : (0.78,0.69) (0.82,0.69)
#
...
J_MAX="${J_MAX:-12}"
RUN_TAG="${RUN_TAG:-hopf_bcenter_skew_expected_$(date +%Y%m%d_%H%M%S)}"
...
ROUNDS="${ROUNDS:-8000}"
AB_PAIRS="${AB_PAIRS:-0.76:0.67 0.78:0.68 0.80:0.69 0.82:0.70 0.84:0.71 0.80:0.70 0.82:0.71 0.84:0.72 0.86:0.73 0.78:0.69 0.82:0.69}"
...
printf "seed\ta\tb\trounds\tout\n" > "$OUT_DIR/manifest_skew_band.tsv"

read -r -a SEED_ARR <<< "$SEEDS"
read -r -a PAIR_ARR <<< "$AB_PAIRS"

echo "=== Launch skew-band search around B-center ==="
for pair in "${PAIR_ARR[@]}"; do
  ...
  for seed in "${SEED_ARR[@]}"; do
    out="$OUT_DIR/hopf_skew_a${atag}_b${btag}_seed${seed}_r${ROUNDS}.csv"
    printf "%s\t%s\t%s\t%s\t%s\n" "$seed" "$aval" "$bval" "$ROUNDS" "$out" >> "$OUT_DIR/manifest_skew_band.tsv"
...
echo "CSV     : $OUT_DIR"
echo "LOG     : $LOG_DIR"
echo "MANIFEST: $OUT_DIR/manifest_skew_band.tsv"