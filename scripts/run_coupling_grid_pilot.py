#!/usr/bin/env python3
"""Pilot Coupling-Grid for personality_coupled: scan lambda_mu, lambda_k, selection_strength.

This is a lightweight pilot: small seed set to validate whether control_none can reach L3.
Do NOT execute automatically from this script—user triggers it when ready.
"""
...
def _run_one(
    *,
    ...
    tag = f"lm{lambda_mu:.2f}_lk{lambda_k:.2f}_k{selection_strength:.3f}"
...
def main() -> int:
    p = argparse.ArgumentParser(description="Pilot coupling-grid (personality_coupled)")
    ...
    lambda_mus = [0.1, 0.3, 0.5]
    lambda_ks = [0.2, 0.4, 0.6]
    selection_strengths = [0.03, 0.05, 0.07]

    out_dir = ROOT / "outputs" / "coupling_grid_pilot"
    out_dir.mkdir(parents=True, exist_ok=True)

    conditions = ["control_none", "random_9persona"]
    all_results: list[RunMetric] = []

    total = len(lambda_mus) * len(lambda_ks) * len(selection_strengths) * len(conditions) * len(seeds)
    i = 0
    print(f"Pilot Coupling-Grid: runs={total}")

    for lm in lambda_mus:
        for lk in lambda_ks:
            for k_base in selection_strengths:
                combo_metrics: list[RunMetric] = []
                for cond in conditions:
                    for seed in seeds:
                        i += 1
                        print(f"[{i}/{total}] lm={lm} lk={lk} k={k_base} {cond} seed={seed}", end=" ", flush=True)
                        m = _run_one(
                            lambda_mu=lm,
                            lambda_k=lk,
                            selection_strength=k_base,
                            condition=cond,
                            seed=seed,
                ...
                combo_json = out_dir / f"lm{lm:.2f}_lk{lk:.2f}_k{k_base:.3f}_summary.json"
                combo_json.write_text(json.dumps({"lambda_mu": lm, "lambda_k": lk, "selection_strength": k_base, "aggregate": agg}, indent=2))

    (out_dir / "summary.json").write_text(json.dumps({"results": [asdict(r) for r in all_results]}, indent=2))
    print(f"Saved pilot summary to {out_dir / 'summary.json'}")
    return 0
