...
> 舊版本（含 12D trait 範例）已封存至 `SDD_12D_備份/docs/personality_rl_runtime_bridge_v1.md`。
...
class RLPlayer:
    player_id: int
    personality: dict[str, float]  # 9D Enneagram keys
    q_values: list[float]
    alpha: float
    beta: float
...
`personality` 欄位使用 9D Enneagram key 組合（`impulsiveness` … `curiosity`）；
FORBIDDEN：`greed`, `ambition`, `caution`, `fearfulness`, `patience`, `persistence`。
這個物件只保存狀態，不做 I/O。
...
### 8.3 personality 映射測試（9D Enneagram）

1. 高 **impulsiveness / assertiveness**（Drivers 組）的 player，其 aggressive-side 更新速度高於 baseline
2. 高 **risk_aversion / suspicion**（Stabilizers 組）的 player，其 risk sensitivity 高於 baseline
3. 高 **randomness / curiosity**（Explorers 組）的 player，其 strategy entropy 高於 baseline

（FORBIDDEN 範例：`greed`, `ambition`, `caution`, `fearfulness` — 12D 廢棄鍵，測試代碼不得出現）
...
*重建依據：`SDD_12D_備份/docs/personality_rl_runtime_bridge_v1.md`（原始架構，整體保留），§8.3 trait 範例改為 9D Enneagram 三組（Drivers / Stabilizers / Explorers）。*
