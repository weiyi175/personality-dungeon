...
> 舊版本（12D）已封存至 `SDD_12D_備份/docs/event_json_format_guide.md`。
"""
...
`dimensions_order` 與 `_PERSONALITY_KEYS_ORDERED`（`simulation/personality_rl_runtime.py` L146–149）保持一致：
...
**FORBIDDEN**：`weights` array 長度必須為 9；舊版 12D template（weights 長度 12 或含 greed/ambition 等鍵）已封存至 `SDD_12D_備份/`。
...
```json
{
    ...
    "actions": [
        {
            "name": "attack",
            "weights": [0.9, 0.7, 0.2, -0.8, -0.3, 0.1, 0.0, -0.5, 0.0],
            // ^ 9 elements: [impulsiveness, assertiveness, optimism,
            //                risk_aversion, suspicion, endurance,
            //                randomness, stability_seeking, curiosity]
            "base_risk": 0.72,
            "risk_model": {
...
- ✅ **EventBridge**（simulation/personality_rl_runtime.py 行 152–192）
  ...
  - **`_PERSONALITY_KEYS_ORDERED` 於 2026-05-18（EventBridge fix）已更新為 9D**

- ✅ **Event 模板檔案**
  - `02_event_templates_smoke_v1.json`（v2.0，9D weights，dimensions_order 已驗證）
  - `02_event_templates_v1.json`（完整版）
...
- ✅ **Event 事件在 simulation 中的應用**
  ...
  - event_rate 控制事件觸發機率（B1 實驗結論：0.08 為最佳；以 `*_9d_*` output 為準）
...
# personality 必須使用 9D Enneagram key 組合
personality = {
    "impulsiveness": 0.5, "assertiveness": 0.3, "optimism": 0.2,
    "risk_aversion": -0.3, "suspicion": 0.0, "endurance": 0.1,
    "randomness": 0.4, "stability_seeking": -0.2, "curiosity": 0.6,
}  # 9D 向量
...
def test_reward_risk_not_nan(self):
    ...
    rng = random.Random(7)
    ev = bridge.sample_event(rng=rng)
    pers = sample_personality(rng=rng)  # 回傳 9D dict
...
| ✅ EventBridge 採樣器（9D） | 實現完成 | simulation/personality_rl_runtime.py:152-192 |
| ✅ Event Template JSON（smoke, v2.0, 9D） | 已有檔案 | docs/personality_dungeon_v1/02_event_templates_smoke_v1.json |
| ✅ Event Template JSON（完整版） | 已有檔案 | docs/personality_dungeon_v1/02_event_templates_v1.json |
...
1. **Server 端點整合**
   ...
   - 根據 event_rate 決定是否觸發事件（建議 0.08，以 B1 r008 `*_9d_*` output 為準）
...
*重建依據：`SDD_12D_備份/docs/event_json_format_guide.md`（原始架構），`02_event_templates_smoke_v1.json`（9D 正典 weights），`simulation/personality_rl_runtime.py::_PERSONALITY_KEYS_ORDERED`（2026-05-18 fix）。*
