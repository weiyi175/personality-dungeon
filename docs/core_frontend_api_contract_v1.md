...
> 舊版本（12D personality basis）已封存至 `SDD_12D_備份/docs/core_frontend_api_contract_v1.md`。
> `api/core_frontend_api_contract_v1.md` 中的 12D 版本亦為歷史存檔，請以本文件為準。
...
v1 的 canonical personality basis 使用 **9D Enneagram**（三組各 3 維）：

| # | Key | 組別 |
|---|-----|------|
| 1 | `impulsiveness` | Drivers（aggressive） |
| 2 | `assertiveness` | Drivers（aggressive） |
| 3 | `optimism` | Drivers（aggressive） |
| 4 | `risk_aversion` | Stabilizers（defensive） |
| 5 | `suspicion` | Stabilizers（defensive） |
| 6 | `endurance` | Stabilizers（defensive） |
| 7 | `randomness` | Explorers（balanced） |
| 8 | `stability_seeking` | Explorers（balanced） |
| 9 | `curiosity` | Explorers（balanced） |

**FORBIDDEN**：`greed`, `ambition`, `caution`, `fearfulness`, `patience`, `persistence`（12D 廢棄鍵）。

建議欄位如下：

1. `vector`：物件，內含上述 9 維的 delta（僅包含有變動的鍵）。
2. `norm`：delta 的摘要大小。
...
4. `utility`：可為任意浮點數。
5. `alive`：boolean。
6. `personality`：完整 9D Enneagram 人格快照（9 鍵）。
7. `memory_load`：可選，範圍 `[0, 1]`。

規格要求：

1. `personality` 必須與 `personality_delta` 使用同一套 canonical basis（9D，見 §6.3）。
...
*重建依據：`SDD_12D_備份/docs/core_frontend_api_contract_v1.md`（原始架構，整體保留），§6.3 / §6.6 / §9 personality fields 改為 9D Enneagram（impulsiveness…curiosity）。*
