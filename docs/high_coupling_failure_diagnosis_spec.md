...
Layer 1: Sampling noise
  每輪 p(t) = mean(sampled_strategies) 帶有 O(1/√N) 高頻抖動
  
Layer 2: Popularity averaging  
  x(t) = (p(t-1) + p(t-2) + ...) / memory → 進一步平滑
  
Layer 3: Synchronized replicator
  w(t+1) ∝ w(t) × f(x(t))
  所有玩家同步更新 → 全場同相位推進 → 旋轉訊號消滅
```
...
| B5 (tangential drift) | Layer 3（直接注入旋轉） | deterministic gate 通過（保持 L3），但 sampled gate 全滅 | drift 被三層均化吸收，tangential alignment ≈ -0.01（反向！） |

**一句話結論**：sampled discrete **synchronous** update 的三層均化對旋轉訊號的耐損性是 **exponential**（不是 linear），因此任何單一層的局部修正都無法有效對抗。
