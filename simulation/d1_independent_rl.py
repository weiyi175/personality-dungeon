"""D1 Independent Reinforcement Learning – G2 Short Scout Harness.

Phase 10 experiment: each player maintains independent Q-table and learns
from own reward only. No imitation, no broadcast.

Architecture
------------
- Standalone simulation loop (does NOT use simulate()).
- Uses evolution/independent_rl.py for RL update mechanics.
- Uses evolution/local_graph.py for topology construction.
- analysis/ layer for cycle_metrics + decay_rate.

Usage
-----
    ./venv/bin/python -m simulation.d1_independent_rl \\
        --graph-topologies lattice4,well_mixed \\
        --alphas 0.01,0.05,0.10 \\
        --softmax-betas 5.0,10.0 \\
        --seeds 45,47,49
"""
...
DEFAULT_OUT_ROOT = Path("outputs/d1_independent_rl")
DEFAULT_SUMMARY_TSV = DEFAULT_OUT_ROOT / "d1_summary.tsv"
DEFAULT_COMBINED_TSV = DEFAULT_OUT_ROOT / "d1_combined.tsv"
DEFAULT_DECISION_MD = DEFAULT_OUT_ROOT / "d1_decision.md"

SUMMARY_FIELDNAMES = [
    "gate", "condition", "topology", "alpha", "softmax_beta", "seed",
...
COMBINED_FIELDNAMES = [
    "gate", "condition", "topology", "alpha", "softmax_beta",
    "is_control", "n_seeds",
...
def _cond_name(*, topology: str, alpha: float, beta: float) -> str:
    a_tok = f"{alpha:.2f}".replace(".", "p")
    b_tok = f"{beta:.1f}".replace(".", "p")
    return f"g2_d1_{topology}_a{a_tok}_b{b_tok}"

def _ctrl_name(topology: str) -> str:
    return f"g2_d1_frozen_{topology}"
...
def _run_d1_simulation(
    *,
    ...
    for t in range(int(n_rounds)):
        ...
        # --- 1. Boltzmann strategy selection ---
        chosen: list[int] = []
        for i in range(n):
            chosen.append(boltzmann_select(q_tables[i], beta=beta, rng=rng))
        ...
        # --- 3. Q-update (independent, no synchronisation needed) ---
        for i in range(n):
            q_tables[i] = rl_q_update(q_tables[i], chosen[i], rewards[i], alpha=alp)
        ...
        # --- 5. Diagnostics ---
        # Per-player weights from softmax(q)
        all_w: list[list[float]] = [boltzmann_weights(q_tables[i], beta=beta) for i in range(n)]
    ...
    return global_rows, round_diag, q_tables, adj
...
def _seed_metrics(
    rows: list[dict[str, Any]], *, burn_in: int, tail: int, eta: float, corr_threshold: float,
) -> dict[str, float | int]:
    sm = {
...
def _tail_d1_diagnostics(
    round_diag: list[dict[str, Any]],
    final_q: list[list[float]],
    adj: list[list[int]] | None,
    *, n_rows: int, burn_in: int, tail: int, softmax_beta: float,
) -> dict[str, float]:
    ...
    mesd = 0.0
    if adj is not None:
        final_w = [boltzmann_weights(q, beta=float(softmax_beta)) for q in final_q]
...
def _write_simplex_plot(
    rows: list[dict[str, Any]], *, out_png: Path, title: str, burn_in: int, tail: int,
) -> None:
    try:
    ...
    if len(tail_rows) < 2:
        return
    pts = [_simplex_xy(float(r["p_aggressive"]), float(r["p_defensive"]), float(r["p_balanced"])) for r in tail_rows]
    ...
    ax.set_title(title, fontsize=9); ax.set_aspect("equal", adjustable="box")
...
def _write_phase_amplitude_plot(
    ...
    phases = np.unwrap(np.array([_phase_angle(float(r["p_aggressive"]), float(r["p_defensive"]), float(r["p_balanced"])) for r in tail_rows]))
    amps = np.array([_amplitude(float(r["p_aggressive"]), float(r["p_defensive"]), float(r["p_balanced"])) for r in tail_rows])
    out_png.parent.mkdir(parents=True, exist_ok=True)
    fig, axes = plt.subplots(2, 1, figsize=(8, 5.5), sharex=True, constrained_layout=True)
    axes[0].plot(rds, phases, color="#1f77b4", lw=1.5); axes[0].set_ylabel("phase angle"); axes[0].set_title(title); axes[0].grid(alpha=0.25)
    axes[1].plot(rds, amps, color="#d62728", lw=1.5); axes[1].set_ylabel("amplitude"); axes[1].set_xlabel("round"); axes[1].grid(alpha=0.25)
    fig.savefig(out_png, dpi=160); plt.close(fig)
...
def _build_summary(
    *, condition: str, topology: str, alpha: float, beta: float,
    is_control: bool, metric_rows: list[dict[str, Any]], diag_rows: list[dict[str, float]],
    ...
    return {
        ...
        "alpha": _fmt(alpha), "softmax_beta": _fmt(beta),
        ...
        "representative_seed": "", "representative_simplex_png": "", "representative_phase_amplitude_png": "",
...
def _write_decision(
    path: Path, *, combined: list[dict[str, Any]], candidates: list[dict[str, Any]], close_d1: bool,
) -> None:
    lines = ["# D1 Independent RL Decision", "", "## G2 Short Scout", ""]
    for r in combined:
        lines.append(
            f"- {r['condition']}: topo={r['topology']} α={r['alpha']} β={r['softmax_beta']}"
            ...
            f" nbr_cos={r['mean_neighbor_weight_cosine']}"
    ...
    if not candidates:
        ...
        for r in candidates:
            lines.append(f"- longer_confirm_candidate: {r['condition']} entropy={r['mean_player_weight_entropy']} q_std={r['mean_q_value_std']}")
    lines += ["", "## Pass Gate (4-way AND)", ""]
    lines += ["1. level3_seed_count ≥ 2", "2. mean_env_gamma ≥ 0", "3. mean_player_weight_entropy ≥ 1.18", "4. mean_q_value_std > 0.08"]
    lines += ["", "## Stop Rule", ""]
    lines.append("- If ALL active conditions hard-stop fail, D1 closes as negative result.")
    lines.append("- Interpretation: independent per-player RL is insufficient to open Level 3 basin.")
    lines.append(f"- overall_verdict: {'close_d1' if close_d1 else 'keep_d1_open'}")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    *,
    ...
    for topo in topologies:
        ...
        conditions.append({
            ...
            "alpha": 0.0, "softmax_beta": 5.0, "is_control": True,
        })
    # Active conditions
    for topo in topologies:
        gs = _make_graph_spec(topo)
        for alp in alphas:
            for sb in softmax_betas:
                conditions.append({
                    "condition": _cond_name(topology=topo, alpha=alp, beta=sb),
                    "topology": topo, "graph_spec": gs,
                    "alpha": float(alp), "softmax_beta": float(sb), "is_control": False,
                })
    ...
    for cond in conditions:
        ...
        gs = cond["graph_spec"]
        alp = float(cond["alpha"])
        sb = float(cond["softmax_beta"])
        ...
        for seed in seeds:
            print(f"  D1 {cn} seed={seed} ...")
            rows, rdiag, fq, adj_out = _run_d1_simulation(
                n_players=int(players), n_rounds=int(rounds), seed=int(seed),
                graph_spec=gs, alpha=alp, softmax_beta=sb,
                a=float(a), b=float(b), cross=float(cross), init_q=float(init_q),
            )
            ...
            sm = _seed_metrics(rows, burn_in=burn_in, tail=tail, eta=eta, corr_threshold=corr_threshold)
            td = _tail_d1_diagnostics(
                rdiag, fq, adj_out,
                n_rows=len(rows), burn_in=burn_in, tail=tail, softmax_beta=sb,
            )

            prov = {
                "condition": cn, "topology": topo, "alpha": alp, "softmax_beta": sb,
                "seed": int(seed),
                "config": {
                ...
                "cycle_metrics": dict(sm), "d1_diagnostics": dict(td),
            }
            ...
            seed_row = {
                ...
                "alpha": _fmt(alp), "softmax_beta": _fmt(sb), "seed": int(seed),
        ...
        all_combined.append(_build_summary(
            condition=cn, topology=topo, alpha=alp, beta=sb,
    ...
    for cr in all_combined:
        ...
        _write_simplex_plot(cond_ts[cn][rs], out_png=sp_png, title=f"{cn} s={rs}", burn_in=burn_in, tail=tail)
        _write_phase_amplitude_plot(cond_ts[cn][rs], out_png=pp_png, title=f"{cn} s={rs}", burn_in=burn_in, tail=tail)
    ...
    close_d1 = bool(active) and all(str(r["hard_stop_fail"]) == "yes" for r in active)

    candidates = [r for r in all_combined if str(r["verdict"]) == "pass"]
    candidates.sort(key=lambda r: (int(r["level3_seed_count"]), float(r["mean_q_value_std"])), reverse=True)
    ...
    _write_decision(decision_md, combined=all_combined, candidates=candidates, close_d1=close_d1)

    print(f"summary_tsv={summary_tsv}")
    print(f"combined_tsv={combined_tsv}")
    print(f"decision_md={decision_md}")
    print(f"close_d1={close_d1}")

    return {
        "summary_tsv": str(summary_tsv), "combined_tsv": str(combined_tsv),
        "decision_md": str(decision_md), "close_d1": bool(close_d1),
...
def main() -> None:
    p = argparse.ArgumentParser(description="D1 Independent RL G2 Scout")
    p.add_argument("--seeds", type=str, default="45,47,49")
    p.add_argument("--graph-topologies", type=str, default="lattice4,well_mixed")
    p.add_argument("--alphas", type=str, default="0.01,0.05,0.10")
    p.add_argument("--softmax-betas", type=str, default="5.0,10.0")
    ...
    run_d1_scout(
        ...
        softmax_betas=_parse_floats(args.softmax_betas),
