"""H1 payoff-niche – G2 scout harness.

Follows SDD §(H1) protocol lock.  CLI-compatible with the B/A/S/M/L2-series harness pattern.
Per-group diagonal payoff niche bonus: each niche group gets +ε on its
assigned strategy's reward.  Combined with L2-style per-group evolution.
"""
...
DEFAULT_OUT_ROOT = REPO_ROOT / "outputs" / "h1_payoff_niche"
DEFAULT_SUMMARY_TSV = REPO_ROOT / "outputs" / "h1_payoff_niche_summary.tsv"
DEFAULT_COMBINED_TSV = REPO_ROOT / "outputs" / "h1_payoff_niche_combined.tsv"
DEFAULT_DECISION_MD = REPO_ROOT / "outputs" / "h1_payoff_niche_decision.md"

GATE = "g2_sampled"

SUMMARY_FIELDNAMES = [
	"condition",
	"payoff_niche_epsilon",
	"niche_group_size",
	"seed",
	"cycle_level",
	"stage3_score",
	...
	"has_level3_seed",
	"mean_inter_group_growth_cosine",
	"mean_inter_group_weight_std",
...
def _condition_name(*, payoff_niche_epsilon: float, niche_group_size: int) -> str:
	eps_str = f"{float(payoff_niche_epsilon):.2f}".replace(".", "p")
	return f"{GATE}_niche{eps_str}_grp{int(niche_group_size):04d}"


def _control_condition_name() -> str:
	return _condition_name(payoff_niche_epsilon=0.0, niche_group_size=0)
...
class _H1RoundDiagnostics:
	"""Collect per-round H1 diagnostics during simulate() via round_callback.

	Key diagnostics:
	- inter_group_growth_cosine: pairwise cosine similarity of growth vectors
	  across groups (mean).  L2 predicts ~1.0; H1 should drive this down.
	- inter_group_weight_std: same as L2.
	"""
	...
	def __init__(
		...
		*,
		...
		seed: int,
		groups: list[list[object]] | None,
	) -> None:
		self.payoff_niche_epsilon = float(payoff_niche_epsilon)
		self.niche_group_size = int(niche_group_size)
		self.seed = int(seed)
		self.groups = groups
	...
	def callback(
		...
		inter_group_std = self._compute_inter_group_weight_std(players)
		inter_group_cosine = self._compute_inter_group_growth_cosine(players)
		self.records.append({
			"strategy_entropy": entropy,
			"inter_group_weight_std": inter_group_std,
			"inter_group_growth_cosine": inter_group_cosine,
		})

	def _compute_inter_group_weight_std(self, players: list[object]) -> float:
		if self.groups is None or len(self.groups) <= 1:
			return 0.0
		strategies = ["aggressive", "defensive", "balanced"]
		per_strategy_stds: list[float] = []
		for s in strategies:
			group_means: list[float] = []
			for group in self.groups:
				vals = [float(getattr(pl, "strategy_weights", {}).get(s, 1.0)) for pl in group]
			...
			per_strategy_stds.append(var ** 0.5)
		return float(sum(per_strategy_stds) / len(per_strategy_stds))

	def _compute_inter_group_growth_cosine(self, players: list[object]) -> float:
		"""Mean pairwise cosine similarity of per-group growth vectors.

		Growth vector per group: for each strategy, mean(last_reward) among
		players whose last_strategy == s, minus overall group mean reward.
		"""
		if self.groups is None or len(self.groups) <= 1:
			return 1.0
		strategies = ["aggressive", "defensive", "balanced"]
		...
		# Pairwise cosine
		n_groups = len(group_growth_vectors)
		cosines: list[float] = []
		for i in range(n_groups):
			for j in range(i + 1, n_groups):
				cosines.append(_cosine_similarity(group_growth_vectors[i], group_growth_vectors[j]))
		return _safe_mean(cosines) if cosines else 1.0

	def finalize(self, *, rows: list[dict[str, Any]], burn_in: int, tail: int) -> dict[str, Any]:
		begin = _tail_begin(len(rows), burn_in=int(burn_in), tail=int(tail))
		window = self.records[begin:]
		return {
			"mean_inter_group_growth_cosine": _safe_mean([float(r["inter_group_growth_cosine"]) for r in window]),
			"mean_inter_group_weight_std": _safe_mean([float(r["inter_group_weight_std"]) for r in window]),
...
def _write_simplex_plot(
	...
	*,
	...
	try:
		...
		raise RuntimeError("matplotlib is required to write H1 simplex plots") from exc
	...
	fig.colorbar(lc, ax=ax, label="tail time")
...
def _write_phase_amplitude_plot(
	...
	*,
	...
	try:
		...
		raise RuntimeError("matplotlib is required to write H1 phase-amplitude plots") from exc
...
def _build_condition_summary(
	*,
	...
	is_ctrl = float(payoff_niche_epsilon) == 0.0 and int(niche_group_size) == 0
	return {
		"condition": condition,
		"payoff_niche_epsilon": _format_float(payoff_niche_epsilon),
		"niche_group_size": int(niche_group_size),
		...
		"gamma_uplift_ratio_vs_control": "",
		"stage3_uplift_vs_control": "",
		"mean_inter_group_growth_cosine": _format_float(_safe_mean([float(r["mean_inter_group_growth_cosine"]) for r in per_seed_rows])),
		"mean_inter_group_weight_std": _format_float(_safe_mean([float(r["mean_inter_group_weight_std"]) for r in per_seed_rows])),
...
def _write_decision(
	...
	*,
	...
	for row in combined_rows:
		lines.append(
			f"- {row['condition']}: epsilon={row['payoff_niche_epsilon']}"
			f" niche_group_size={row['niche_group_size']}"
			f" level3_seed_count={row['level3_seed_count']}"
			f" mean_inter_group_growth_cosine={row['mean_inter_group_growth_cosine']}"
			f" mean_strategy_entropy={row['mean_strategy_entropy']}"
			f" stage3_uplift={row['stage3_uplift_vs_control']}"
			f" mean_inter_group_weight_std={row['mean_inter_group_weight_std']}"
	...
	if not recommended_candidates:
		...
		for row in recommended_candidates:
			lines.append(
				...
				f" epsilon={row['payoff_niche_epsilon']}"
				f" niche_group_size={row['niche_group_size']}"
				f" mean_inter_group_growth_cosine={row['mean_inter_group_growth_cosine']}"
				f" mean_strategy_entropy={row['mean_strategy_entropy']}"
				f" representative_seed={row['representative_seed']}"
			)
	lines.extend(["", "## Stop Rule (H1 4-way Pass Gate)", ""])
	lines.append("- Pass: level3_seed_count >= 2 AND mean_env_gamma >= 0 AND mean_inter_group_growth_cosine <= 0.92 AND strategy_entropy >= 1.12")
	lines.append("- Hard stop: all non-control cells fail all 4 conditions.")
	if close_h1:
		lines.append("- overall_verdict: close_h1")
	else:
		lines.append("- overall_verdict: keep_h1_open")
	path.parent.mkdir(parents=True, exist_ok=True)
	path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run_h1_scout(
	*,
	seeds: list[int],
	payoff_niche_epsilons: list[float],
	niche_group_sizes: list[int],
	out_root: Path,
	summary_tsv: Path,
	combined_tsv: Path,
	decision_md: Path,
	...
	# Build sweep cells: each (epsilon, group_size) pair
	cells: list[tuple[float, int]] = []
	for eps in payoff_niche_epsilons:
		for gs in niche_group_sizes:
			eps_f = float(eps)
			gs_i = int(gs)
			if eps_f == 0.0 and gs_i == 0:
				cells.append((0.0, 0))
			elif eps_f > 0.0 and gs_i > 0:
				cells.append((eps_f, gs_i))
			# skip invalid combos (eps>0, gs=0 or eps=0, gs>0)

	if not any(eps == 0.0 and gs == 0 for eps, gs in cells):
		raise ValueError("H1 short scout requires (epsilon=0.0, niche_group_size=0) for matched control")
	...
	for eps_val, gs_val in cells:
		condition = _condition_name(payoff_niche_epsilon=eps_val, niche_group_size=gs_val)
		...
		for seed in seeds:
			cfg = SimConfig(
				...
				memory_kernel=int(memory_kernel),
				payoff_niche_epsilon=float(eps_val),
				niche_group_size=int(gs_val),
			)

			# Build groups for diagnostics (mirror partition logic in simulate())
			diag_groups_holder: list[list[object] | None] = [None]

			class _GroupCapture:
				def __init__(self, gs: int, holder: list):
					self._gs = gs
					self._holder = holder

				def capture(self, players: list[object]) -> list[list[object]] | None:
					if self._gs > 0 and self._gs < len(players):
						return _partition_players_into_groups(players, self._gs)
					return None

			group_capture = _GroupCapture(gs_val, diag_groups_holder)
			round_diagnostics = _H1RoundDiagnostics(
				payoff_niche_epsilon=float(eps_val),
				niche_group_size=int(gs_val),
				seed=int(seed),
				groups=None,
			...
			_original_cb = round_diagnostics.callback
			_first_call = [True]

			def _wrapped_callback(
				round_index, cfg, players, dungeon, step_records, row,
				_orig=_original_cb, _gc=group_capture, _diag=round_diagnostics, _first=_first_call,
			):
				if _first[0]:
					_diag.groups = _gc.capture(players)
					_first[0] = False
				return _orig(round_index, cfg, players, dungeon, step_records, row)

			strategy_space, rows = simulate(
				cfg,
				round_callback=_wrapped_callback,
			)
			...
			provenance = {
				...
				"round_diagnostics": {
					"mean_inter_group_growth_cosine": float(diag_stats["mean_inter_group_growth_cosine"]),
					"mean_inter_group_weight_std": float(diag_stats["mean_inter_group_weight_std"]),
			...
			per_seed_rows.append({
				"condition": condition,
				"payoff_niche_epsilon": _format_float(eps_val),
				"niche_group_size": int(gs_val),
				"seed": int(seed),
				...
				"mean_inter_group_growth_cosine": _format_float(diag_stats["mean_inter_group_growth_cosine"]),
				"mean_inter_group_weight_std": _format_float(diag_stats["mean_inter_group_weight_std"]),
		...
		all_combined_rows.append(
			_build_condition_summary(
				condition=condition,
				payoff_niche_epsilon=eps_val,
				niche_group_size=gs_val,
	...
	for combined_row in all_combined_rows:
		eps_val = float(combined_row["payoff_niche_epsilon"])
		gs_val = int(combined_row["niche_group_size"])
		...
		mean_cosine = float(combined_row["mean_inter_group_growth_cosine"])
		mean_entropy = float(combined_row["mean_strategy_entropy"])
		is_control = eps_val == 0.0 and gs_val == 0
		...
		# H1 4-way Pass Gate (SDD §(H1) point 6)
		short_scout_pass = (
			not is_control
			and level3_seed_count >= 2
			and mean_env_gamma >= 0.0
			and mean_cosine <= 0.92
			and mean_entropy >= 1.12
		)
		hard_stop_fail = (
			not is_control
			and not short_scout_pass
		)
		if is_control:
			verdict = "control"
	...
	close_h1 = bool(active_rows) and all(str(row["hard_stop_fail"]) == "yes" for row in active_rows)
...
def main() -> None:
	parser = argparse.ArgumentParser(description="H1 payoff-niche G2 scout harness")
	parser.add_argument("--seeds", type=str, default="45,47,49")
	parser.add_argument("--payoff-niche-epsilons", type=str, default="0.0,0.02,0.05,0.10,0.20")
	parser.add_argument("--niche-group-sizes", type=str, default="0,10")
	...
	result = run_h1_scout(
		...
		payoff_niche_epsilons=_parse_float_list(args.payoff_niche_epsilons),
		niche_group_sizes=_parse_int_list(args.niche_group_sizes),
	...
	print(f"close_h1={result['close_h1']}")
