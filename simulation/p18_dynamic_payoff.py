"""P18 Dynamic Payoff Perturbation — periodic re-sampling & ε annealing.

Phase 18 experiment: On the BL1 baseline (well_mixed ε=0.02, wide α, β=3),
test whether dynamic payoff perturbation (periodic re-sampling of per-player
δ vectors, with optional ε annealing) can improve rotation consistency.

Sub-experiments:
    18a — ε annealing:  ε(t) linearly decays from ε_start to ε_end,
                         δ re-sampled every R rounds with current ε(t).
    18b — ε re-sampling: fixed ε=0.02, δ re-sampled every R rounds.

Architecture
------------
- Thin wrapper around E1's _run_e1_simulation (same as F1v2).
- Uses the new epsilon_resample_interval & epsilon_end params (Phase 18).
- CC1 fallback + revised Pass Gate (same as BL1).

Usage
-----
    ./venv/bin/python -m simulation.p18_dynamic_payoff \\
        --sub 18b \\
        --seeds 45,47,49

    ./venv/bin/python -m simulation.p18_dynamic_payoff \\
        --sub 18a \\
        --epsilon-starts 0.04,0.06 \\
        --epsilon-ends 0.01,0.02 \\
        --resample-intervals 500,1000 \\
        --seeds 45,47,49
"""
...
DEFAULT_OUT_ROOT = Path("outputs/p18_dynamic_payoff")
DEFAULT_SUMMARY_TSV = DEFAULT_OUT_ROOT / "p18_summary.tsv"
DEFAULT_COMBINED_TSV = DEFAULT_OUT_ROOT / "p18_combined.tsv"
DEFAULT_DECISION_MD = DEFAULT_OUT_ROOT / "p18_decision.md"
...
def _cond_name(*, epsilon_start: float, epsilon_end: float,
               resample_interval: int) -> str:
    def _tok(v: float) -> str:
        return f"{v:.2f}".rstrip("0").rstrip(".").replace(".", "p")
    return (f"g2_p18_{TOPOLOGY}"
            f"_es{_tok(epsilon_start)}_ee{_tok(epsilon_end)}"
            f"_r{resample_interval}")


# ---------------------------------------------------------------------------
# Strategy cycle stability (copy from F1v2)
# ---------------------------------------------------------------------------
...
def _build_summary(
    *, condition: str, epsilon_start: float, epsilon_end: float,
    resample_interval: int, is_control: bool,
    ...
    return {
        ...
        "payoff_epsilon": _fmt(epsilon_start),
        "epsilon_end": _fmt(epsilon_end),
...
def _write_decision(
    ...
    lines = ["# P18 Dynamic Payoff Perturbation Decision", "",
             "## G2 Short Scout", ""]
    for r in combined:
        lines.append(
            f"- {r['condition']}: ε_start={r['payoff_epsilon']}"
            f" ε_end={r['epsilon_end']} R={r['resample_interval']}"
    ...
    lines += ["", "## Stop Rule", ""]
    lines.append("- If ALL active conditions have L3=0, P18 closes.")
    lines.append(f"- overall_verdict: {'close_p18' if all_fail else 'keep_p18_open'}")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


# ---------------------------------------------------------------------------
# Build conditions for sub-experiments
# ---------------------------------------------------------------------------

def _build_conditions_18a(
    *, epsilon_starts: list[float], epsilon_ends: list[float],
    resample_intervals: list[int],
) -> list[dict[str, Any]]:
    """18a: ε annealing — sweep (ε_start, ε_end, R)."""
    conds: list[dict[str, Any]] = []
    for es in epsilon_starts:
        for ee in epsilon_ends:
            if ee >= es:
                continue  # annealing means ε decreases
            for ri in resample_intervals:
                conds.append({
                    "epsilon_start": float(es),
                    "epsilon_end": float(ee),
                    "resample_interval": int(ri),
                    "is_control": False,
                })
    return conds


def _build_conditions_18b(*, resample_intervals: list[int]) -> list[dict[str, Any]]:
    """18b: fixed ε re-sampling — sweep R only."""
    conds: list[dict[str, Any]] = []
    for ri in resample_intervals:
        conds.append({
            "epsilon_start": PAYOFF_EPSILON,
            "epsilon_end": PAYOFF_EPSILON,  # no annealing
            "resample_interval": int(ri),
            "is_control": False,
        })
    return conds
...
def run_p18_scout(
    *,
    ...
    # Add BL1 control (static, no resample)
    all_conditions = [
        {"epsilon_start": PAYOFF_EPSILON, "epsilon_end": PAYOFF_EPSILON,
         "resample_interval": 0, "is_control": True},
    ] + list(conditions)
    ...
    for cond in all_conditions:
        es = float(cond["epsilon_start"])
        ee = float(cond["epsilon_end"])
        ri = int(cond["resample_interval"])
        is_ctrl = bool(cond["is_control"])

        cn = _cond_name(epsilon_start=es, epsilon_end=ee,
        ...
        for seed in seeds:
            ...
            rows, rdiag, fq, adj_out, p_alphas, p_betas = _run_e1_simulation(
                ...
                entropy_lambda=0.0,
                payoff_epsilon=es,
                epsilon_resample_interval=ri,
                epsilon_end=ee if ee != es else None,
            )
            ...
            prov = {
                ...
                "payoff_epsilon": es, "epsilon_end": ee,
                "resample_interval": ri,
                "alpha_lo": ALPHA_LO, "alpha_hi": ALPHA_HI,
                "seed": int(seed),
                "config": {
        ...
        all_combined.append(_build_summary(
            condition=cn, epsilon_start=es, epsilon_end=ee,
            resample_interval=ri, is_control=is_ctrl,
...
def main() -> None:
    p = argparse.ArgumentParser(description="P18 Dynamic Payoff Perturbation Scout")
    p.add_argument("--sub", type=str, choices=["18a", "18b", "both"],
                   default="both",
                   help="Sub-experiment: 18a (annealing), 18b (re-sampling), or both")
    ...
    # 18a params
    p.add_argument("--epsilon-starts", type=str, default="0.04,0.06",
                   help="18a: comma-separated ε_start values")
    p.add_argument("--epsilon-ends", type=str, default="0.01,0.02",
                   help="18a: comma-separated ε_end values")
    # Shared
    p.add_argument("--resample-intervals", type=str, default="500,1000",
                   help="Comma-separated resample intervals (rounds)")
    ...
    if sub in ("18a", "both"):
        conditions.extend(_build_conditions_18a(
            epsilon_starts=_parse_floats(args.epsilon_starts),
            epsilon_ends=_parse_floats(args.epsilon_ends),
            resample_intervals=_parse_ints(args.resample_intervals),
        ))

    if sub in ("18b", "both"):
        conditions.extend(_build_conditions_18b(
            resample_intervals=_parse_ints(args.resample_intervals),
        ))

    if not conditions:
        print("No conditions generated. Check --sub and parameters.")
        return

    print(f"P18 Dynamic Payoff: {len(conditions)} conditions × {len(_parse_seeds(args.seeds))} seeds")
    print(f"  sub={sub}")
    for c in conditions:
        print(f"  ε_start={c['epsilon_start']}, ε_end={c['epsilon_end']}, R={c['resample_interval']}")
