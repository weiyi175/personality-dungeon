...
from evolution.replicator_dynamics import apply_tangential_drift, sampled_growth_vector, tangential_drift_vector
...
DEFAULT_OUT_ROOT = REPO_ROOT / "outputs" / "b5_tangential_drift"
DEFAULT_SUMMARY_TSV = REPO_ROOT / "outputs" / "b5_tangential_drift_summary.tsv"
DEFAULT_COMBINED_TSV = REPO_ROOT / "outputs" / "b5_tangential_drift_combined.tsv"
DEFAULT_DECISION_MD = REPO_ROOT / "outputs" / "b5_tangential_drift_decision.md"
...
SUMMARY_FIELDNAMES = [
	...
	"has_level3_seed",
	"g1_gate_pass",
	"drift_contribution_ratio",
	"mean_drift_norm",
	"mean_effective_delta_growth_ratio",
...
COMBINED_FIELDNAMES = [
	...
	"representative_drift_vector_rose_png",
	"enable_events",
	"events_json",
	"memory_kernel",
	"players",
...
def _condition_name(*, gate: str, delta: float) -> str:
	token = f"{float(delta):.3f}".replace("-", "m").replace(".", "p")
	return f"{gate}_delta{token}"


def _control_condition_name(*, gate: str) -> str:
	return _condition_name(gate=gate, delta=0.0)
...
def _vector_xy(vector: dict[str, float]) -> tuple[float, float]:
	v_a = float(vector.get("aggressive", 0.0))
	v_d = float(vector.get("defensive", 0.0))
	v_b = float(vector.get("balanced", 0.0))
	return (v_d - v_b, v_a - 0.5 * (v_d + v_b))
...
def _vec_dot(left: dict[str, float], right: dict[str, float]) -> float:
	return sum(float(left.get(key, 0.0)) * float(right.get(key, 0.0)) for key in ("aggressive", "defensive", "balanced"))
...
def _vec_cosine(left: dict[str, float], right: dict[str, float]) -> float:
	left_norm = _vec_norm(left)
	right_norm = _vec_norm(right)
	if left_norm <= 1e-12 or right_norm <= 1e-12:
		return 0.0
	return float(_vec_dot(left, right) / (left_norm * right_norm))
...
def _write_simplex_plot(rows: list[dict[str, Any]], *, out_png: Path, title: str, burn_in: int, tail: int) -> None:
	try:
		...
		raise RuntimeError("matplotlib is required to write B5 simplex plots") from exc
...
def _write_phase_amplitude_plot(rows: list[dict[str, Any]], *, out_png: Path, title: str, burn_in: int, tail: int) -> None:
	try:
		...
		raise RuntimeError("matplotlib is required to write B5 phase-amplitude plots") from exc
...
def _write_drift_vector_rose_plot(
	drift_vectors: list[dict[str, float]],
	*,
	out_png: Path,
	title: str,
) -> None:
	try:
		...
		raise RuntimeError("matplotlib is required to write B5 drift rose plots") from exc

	angles: list[float] = []
	weights: list[float] = []
	for vector in drift_vectors:
		x, y = _vector_xy(vector)
		magnitude = float((x * x + y * y) ** 0.5)
		if magnitude <= 1e-12:
			continue
		angles.append(float(atan2(y, x)) % (2.0 * pi))
		weights.append(magnitude)
	out_png.parent.mkdir(parents=True, exist_ok=True)
	fig, ax = plt.subplots(figsize=(6, 6), subplot_kw={"projection": "polar"}, constrained_layout=True)
	if not angles:
		ax.text(0.5, 0.5, "no drift", transform=ax.transAxes, ha="center", va="center")
		ax.set_title(title)
		fig.savefig(out_png, dpi=160)
		plt.close(fig)
		return
	bins = np.linspace(0.0, 2.0 * pi, 25)
	ax.hist(np.array(angles, dtype=float), bins=bins, weights=np.array(weights, dtype=float), color="#1f77b4", alpha=0.8)
	ax.set_title(title)
	fig.savefig(out_png, dpi=160)
	plt.close(fig)


class _B5SampledRoundDiagnostics:
	...
	def callback(
		...
		drift = tangential_drift_vector(simplex, strategy_space, self.delta)
		_, diagnostics = apply_tangential_drift(
			growth,
			strategy_space,
			simplex=simplex,
			delta=self.delta,
		...
		self.records.append(
			{
				"drift": {strategy: float(drift[strategy]) for strategy in strategy_space},
				"drift_norm": float(diagnostics["drift_norm"]),
				"effective_delta_growth_ratio": float(diagnostics["effective_delta_growth_ratio"]),
			}
		)

	def finalize(self, *, rows: list[dict[str, Any]], burn_in: int, tail: int) -> dict[str, Any]:
		begin = _tail_begin(len(rows), burn_in=int(burn_in), tail=int(tail))
		window_records = self.records[begin:]
		alignments: list[float] = []
		for idx in range(begin, min(len(self.records), max(0, len(rows) - 1))):
			current = _simplex_from_row(rows[idx])
			next_simplex = _simplex_from_row(rows[idx + 1])
			delta_simplex = {
				strategy: float(next_simplex[strategy] - current[strategy])
				for strategy in current.keys()
			}
			alignments.append(_vec_cosine(self.records[idx]["drift"], delta_simplex))
		return {
			"drift_contribution_ratio": _mean_ratio([float(record["effective_delta_growth_ratio"]) for record in window_records]),
			"mean_drift_norm": _safe_mean([float(record["drift_norm"]) for record in window_records]),
			"mean_effective_delta_growth_ratio": _mean_ratio([float(record["effective_delta_growth_ratio"]) for record in window_records]),
			"mean_tangential_alignment": _safe_mean(alignments),
			"phase_amplitude_stability": _phase_amplitude_stability(rows, burn_in=int(burn_in), tail=int(tail)),
			"tail_drift_vectors": [dict(record["drift"]) for record in window_records],
		}
...
def _compute_mean_field_round_diagnostics(
	*,
	...
	mean_weight = sum(float(value) for value in weights.values()) / float(len(weights)) if weights else 1.0
	weights = {strategy: float(weights[strategy]) / float(mean_weight) for strategy in strategy_space}
	...
	for row in rows:
		...
		u_bar = sum(float(x_cur[strategy]) * float(payoff[strategy]) for strategy in strategy_space)
		growth = {strategy: float(payoff[strategy]) - float(u_bar) for strategy in strategy_space}
		drift = tangential_drift_vector(x_cur, strategy_space, float(cfg.tangential_drift_delta))
		_, diagnostics = apply_tangential_drift(
			growth,
			strategy_space,
			simplex=x_cur,
			delta=float(cfg.tangential_drift_delta),
		)
		x_next = _simplex_from_row(row)
		delta_simplex = {
			strategy: float(x_next[strategy] - x_cur[strategy])
	...
	begin = _tail_begin(len(rows), burn_in=int(burn_in), tail=int(tail))
	window_records = records[begin:]
	return {
		"drift_contribution_ratio": _mean_ratio([float(record["effective_delta_growth_ratio"]) for record in window_records]),
		"mean_drift_norm": _safe_mean([float(record["drift_norm"]) for record in window_records]),
		"mean_effective_delta_growth_ratio": _mean_ratio([float(record["effective_delta_growth_ratio"]) for record in window_records]),
		"mean_tangential_alignment": _safe_mean([_vec_cosine(record["drift"], record["delta_simplex"]) for record in window_records]),
		"phase_amplitude_stability": _phase_amplitude_stability(rows, burn_in=int(burn_in), tail=int(tail)),
		"tail_drift_vectors": [dict(record["drift"]) for record in window_records],
	}
...
def _build_condition_summary(
	*,
	...
	return {
		"gate": str(gate),
		"condition": condition,
		"tangential_drift_delta": _format_float(delta),
		"is_control": _yes_no(float(delta) == 0.0),
		...
		"g1_drift_gate_pass": "",
		"g1_gate_pass": "",
		"drift_contribution_ratio": _format_ratio(_mean_ratio([float(row["drift_contribution_ratio"]) for row in per_seed_rows])),
		"mean_drift_norm": _format_float(_safe_mean([float(row["mean_drift_norm"]) for row in per_seed_rows])),
		"mean_effective_delta_growth_ratio": _format_ratio(_mean_ratio([float(row["mean_effective_delta_growth_ratio"]) for row in per_seed_rows])),
		"mean_tangential_alignment": _format_float(_safe_mean([float(row["mean_tangential_alignment"]) for row in per_seed_rows])),
		"phase_amplitude_stability": _format_float(_safe_mean([float(row["phase_amplitude_stability"]) for row in per_seed_rows])),
		"short_scout_pass": "",
		"hard_stop_fail": "",
		"longer_confirm_candidate": "no",
		"verdict": "control" if float(delta) == 0.0 else "pending",
		"representative_seed": "",
		"representative_simplex_png": "",
		"representative_phase_amplitude_png": "",
		"representative_drift_vector_rose_png": "",
...
def _representative_seed_row(rows: list[dict[str, Any]]) -> dict[str, Any]:
	return max(
		...
		key=lambda row: (
			...
			float(row["mean_effective_delta_growth_ratio"]),
			-int(row["seed"]),
		),
	)
...
def _write_decision(
	...
	*,
	...
	for row in g1_rows:
		lines.append(
			f"- {row['condition']}: delta={row['tangential_drift_delta']} turn_ratio_vs_control={row['g1_turn_ratio_vs_control']} drift_contribution_ratio={row['drift_contribution_ratio']} g1_level_gate_pass={row['g1_level_gate_pass']} g1_turn_gate_pass={row['g1_turn_gate_pass']} g1_drift_gate_pass={row['g1_drift_gate_pass']} g1_gate_pass={row['g1_gate_pass']} verdict={row['verdict']}"
		)
	lines.extend(["", "## G2 Short Scout", ""])
	for row in g2_rows:
		lines.append(
			f"- {row['condition']}: delta={row['tangential_drift_delta']} g1_gate_pass={row['g1_gate_pass']} level3_seed_count={row['level3_seed_count']} stage3_uplift={row['stage3_uplift_vs_control']} mean_drift_norm={row['mean_drift_norm']} mean_effective_delta_growth_ratio={row['mean_effective_delta_growth_ratio']} mean_tangential_alignment={row['mean_tangential_alignment']} phase_amplitude_stability={row['phase_amplitude_stability']} drift_rose_png={row['representative_drift_vector_rose_png']} short_scout_pass={row['short_scout_pass']} hard_stop_fail={row['hard_stop_fail']} verdict={row['verdict']}"
		)
	...
	if not recommended_candidates:
		...
		for row in recommended_candidates:
			lines.append(
				f"- longer_confirm_candidate: {row['condition']} delta={row['tangential_drift_delta']} stage3_uplift={row['stage3_uplift_vs_control']} mean_tangential_alignment={row['mean_tangential_alignment']} representative_seed={row['representative_seed']} drift_rose_png={row['representative_drift_vector_rose_png']}"
			)
	lines.extend(["", "## Stop Rule", ""])
	lines.append("- G1: 若 turn_strength < 0.92 × baseline、cycle level 掉出 3、或 drift_contribution_ratio > 0.05，該 delta 不得進 G2。")
	lines.append("- G2: 若 delta <= 0.010 的 active cells 全部 0/3 Level 3 且 mean_stage3_score uplift < 0.02，B5 直接 closure。")
	if close_b5:
		lines.append("- overall_verdict: close_b5")
	else:
		lines.append("- overall_verdict: keep_b5_open")
	path.parent.mkdir(parents=True, exist_ok=True)
	path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run_b5_scout(
	*,
	seeds: list[int],
	tangential_drift_deltas: list[float],
	out_root: Path,
	summary_tsv: Path,
	combined_tsv: Path,
	decision_md: Path,
	...
	if 0.0 not in {float(delta) for delta in tangential_drift_deltas}:
		raise ValueError("B5 short scout requires tangential_drift_deltas to include 0.0 for matched controls")
	...
	condition_drift_vectors: dict[str, dict[int, list[dict[str, float]]]] = {}
	...
	for gate_spec in gate_specs:
		...
		for delta in tangential_drift_deltas:
			condition = _condition_name(gate=gate, delta=float(delta))
			out_dir = out_root / condition
			out_dir.mkdir(parents=True, exist_ok=True)
			per_seed_rows: list[dict[str, Any]] = []
			metric_rows: list[dict[str, Any]] = []
			seed_rows_map: dict[int, list[dict[str, Any]]] = {}
			seed_drift_vectors: dict[int, list[dict[str, float]]] = {}
			...
			for seed in seeds:
				cfg = SimConfig(
					...
					memory_kernel=int(memory_kernel),
					tangential_drift_delta=float(delta),
				)
				round_diagnostics = _B5SampledRoundDiagnostics(delta=float(delta)) if gate == G2_GATE else None
				strategy_space, rows = simulate(cfg, round_callback=(round_diagnostics.callback if round_diagnostics is not None else None))
				...
				if gate == G2_GATE:
					...
					diag_stats = _compute_mean_field_round_diagnostics(cfg=cfg, rows=rows, burn_in=int(burn_in), tail=int(tail))
				seed_drift_vectors[int(seed)] = list(diag_stats["tail_drift_vectors"])
				provenance = {
					"gate": gate,
					"condition": condition,
					"tangential_drift_delta": float(delta),
					...
					"round_diagnostics": {
						"drift_contribution_ratio": float(diag_stats["drift_contribution_ratio"]),
						"mean_drift_norm": float(diag_stats["mean_drift_norm"]),
						"mean_effective_delta_growth_ratio": float(diag_stats["mean_effective_delta_growth_ratio"]),
						"mean_tangential_alignment": float(diag_stats["mean_tangential_alignment"]),
				...
				per_seed_rows.append(
					{
						...
						"g1_gate_pass": "",
						"drift_contribution_ratio": _format_ratio(diag_stats["drift_contribution_ratio"]),
						"mean_drift_norm": _format_float(diag_stats["mean_drift_norm"]),
						"mean_effective_delta_growth_ratio": _format_ratio(diag_stats["mean_effective_delta_growth_ratio"]),
						"mean_tangential_alignment": _format_float(diag_stats["mean_tangential_alignment"]),
			...
			condition_drift_vectors[condition] = seed_drift_vectors
	...
	g1_gate_pass_by_delta: dict[float, bool] = {}
	for combined_row in all_combined_rows:
		gate = str(combined_row["gate"])
		delta = float(combined_row["tangential_drift_delta"])
		control_row = next(
			row for row in all_combined_rows if str(row["gate"]) == gate and str(row["condition"]) == _control_condition_name(gate=gate)
		)
		...
		combined_row["gamma_uplift_ratio_vs_control"] = _format_ratio(1.0 if float(delta) == 0.0 else gamma_ratio)
		combined_row["stage3_uplift_vs_control"] = _format_float(0.0 if float(delta) == 0.0 else stage3_uplift)
		if gate == G1_GATE:
			turn_ratio = _turn_ratio(baseline_turn=control_mean_turn_strength, candidate_turn=mean_turn_strength)
			...
			drift_gate_pass = float(combined_row["drift_contribution_ratio"]) <= 0.05
			g1_gate_pass = (float(delta) == 0.0) or (level_gate_pass and turn_gate_pass and drift_gate_pass)
			g1_gate_pass_by_delta[float(delta)] = bool(g1_gate_pass)
			combined_row["g1_turn_ratio_vs_control"] = _format_ratio(1.0 if float(delta) == 0.0 else turn_ratio)
			combined_row["g1_level_gate_pass"] = _yes_no(True if float(delta) == 0.0 else level_gate_pass)
			combined_row["g1_turn_gate_pass"] = _yes_no(True if float(delta) == 0.0 else turn_gate_pass)
			combined_row["g1_drift_gate_pass"] = _yes_no(True if float(delta) == 0.0 else drift_gate_pass)
			...
			combined_row["verdict"] = "control" if float(delta) == 0.0 else ("pass" if g1_gate_pass else "fail")
		else:
			g1_gate_pass = bool(g1_gate_pass_by_delta.get(float(delta), float(delta) == 0.0))
			short_scout_pass = (not float(delta) == 0.0) and g1_gate_pass and level3_seed_count >= 1 and stage3_uplift >= 0.02
			hard_stop_fail = (
				not float(delta) == 0.0
				and g1_gate_pass
				and float(delta) <= 0.010 + 1e-12
			...
			combined_row["g1_drift_gate_pass"] = ""
			combined_row["g1_gate_pass"] = _yes_no(g1_gate_pass)
	...
	for combined_row in all_combined_rows:
		...
		representative_rose_png = Path(combined_row["out_dir"]) / f"seed{representative_seed}_drift_vector_rose.png"
		_write_simplex_plot(
		...
		_write_drift_vector_rose_plot(
			condition_drift_vectors[condition][representative_seed],
			out_png=representative_rose_png,
			title=f"{condition} seed={representative_seed}",
		)
		...
		combined_row["representative_drift_vector_rose_png"] = str(representative_rose_png)

	for condition, per_seed_rows in condition_seed_rows.items():
		gate = str(next(row["gate"] for row in all_combined_rows if str(row["condition"]) == condition))
		...
		g1_pass_lookup = {
			float(row["tangential_drift_delta"]): str(row["g1_gate_pass"])
			for row in all_combined_rows
			if str(row["gate"]) == G1_GATE
		}
		for row in per_seed_rows:
			...
			row["g1_gate_pass"] = g1_pass_lookup.get(float(row["tangential_drift_delta"]), "yes" if float(row["tangential_drift_delta"]) == 0.0 else "no")
	...
	recommended_candidates.sort(
		key=lambda row: (
			...
			float(row["mean_tangential_alignment"]),
		),
		reverse=True,
	)
	active_g2_rows = [
		row
		for row in all_combined_rows
		if str(row["gate"]) == G2_GATE and str(row["is_control"]) == "no" and float(row["tangential_drift_delta"]) <= 0.010 + 1e-12
	]
	close_b5 = bool(active_g2_rows) and all(str(row["hard_stop_fail"]) == "yes" for row in active_g2_rows)
	...
	return {
		...
		"close_b5": bool(close_b5),
	}


def main() -> None:
	parser = argparse.ArgumentParser(description="B5 tangential drift G1/G2 scout harness")
	parser.add_argument("--seeds", type=str, default="45,47,49")
	parser.add_argument("--tangential-drift-deltas", type=str, default="0.000,0.003,0.006,0.010,0.015")
	...
	result = run_b5_scout(
		...
		tangential_drift_deltas=_parse_float_list(args.tangential_drift_deltas),
	...
	print(f"close_b5={result['close_b5']}")
