"""L2 local-group growth – G2 scout harness.

Follows SDD §(L2) protocol lock.  CLI-compatible with the B/A/S/M-series harness pattern.
Partitions N players into fixed sub-groups of size `local_group_size`;
each group computes its own growth vector → breaks LLN smoothing.
"""
...
DEFAULT_OUT_ROOT = REPO_ROOT / "outputs" / "l2_local_growth"
DEFAULT_SUMMARY_TSV = REPO_ROOT / "outputs" / "l2_local_growth_summary.tsv"
DEFAULT_COMBINED_TSV = REPO_ROOT / "outputs" / "l2_local_growth_combined.tsv"
DEFAULT_DECISION_MD = REPO_ROOT / "outputs" / "l2_local_growth_decision.md"
...
def _condition_name(*, local_group_size: int) -> str:
	return f"{GATE}_grp{int(local_group_size):04d}"


def _control_condition_name() -> str:
	return _condition_name(local_group_size=0)
...
class _L2RoundDiagnostics:
	"""Collect per-round L2 diagnostics during simulate() via round_callback.

	Key diagnostic: inter_group_weight_std — for each strategy, compute the
	mean weight per group, then take std across groups; average over strategies.
	"""

	def __init__(self, *, local_group_size: int, seed: int, groups: list[list[object]] | None) -> None:
		self.local_group_size = int(local_group_size)
		self.seed = int(seed)
		self.groups = groups
	...
	def _compute_inter_group_weight_std(self, players: list[object]) -> float:
		"""Mean over strategies of (std across groups of per-group mean weight)."""
...
def _write_simplex_plot(
	...
	*,
	...
	try:
		...
		raise RuntimeError("matplotlib is required to write L2 simplex plots") from exc
...
def _write_phase_amplitude_plot(
	...
	*,
	...
	try:
		...
		raise RuntimeError("matplotlib is required to write L2 phase-amplitude plots") from exc
...
def _build_condition_summary(
	*,
	...
	is_ctrl = int(local_group_size) == 0
	return {
		"condition": condition,
...
def _write_decision(
	...
	*,
	...
	lines = [
		"# L2 Local-Group Growth Decision",
		"",
	...
	for row in combined_rows:
		lines.append(
			f"- {row['condition']}: local_group_size={row['local_group_size']}"
	...
	if not recommended_candidates:
		...
		for row in recommended_candidates:
			lines.append(
				...
				f" local_group_size={row['local_group_size']}"
				f" stage3_uplift={row['stage3_uplift_vs_control']}"
				f" mean_inter_group_weight_std={row['mean_inter_group_weight_std']}"
				f" mean_strategy_entropy={row['mean_strategy_entropy']}"
	...
	lines.append("- G2: 若所有 local_group_size > 0 的 active cells 全部 0/3 Level 3 且 mean_stage3_score uplift < 0.02，L2 直接 closure。")
	if close_l2:
		lines.append("- overall_verdict: close_l2")
	else:
		lines.append("- overall_verdict: keep_l2_open")
	path.parent.mkdir(parents=True, exist_ok=True)
	path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run_l2_scout(
	*,
	...
	if not any(int(gs) == 0 for gs in local_group_sizes):
		raise ValueError("L2 short scout requires local_group_sizes to include 0 for matched controls")
	...
	for gs in local_group_sizes:
		gs_int = int(gs)
		condition = _condition_name(local_group_size=gs_int)
		...
		for seed in seeds:
			cfg = SimConfig(
				...
				memory_kernel=int(memory_kernel),
				local_group_size=gs_int,
			)

			# Build groups for diagnostics (mirror the partition logic in simulate())
			from simulation.run_simulation import _partition_players_into_groups as _ppig
			# We need the actual player objects to compute inter-group stats.
			# The round_callback receives players, so we store group references from
			# the first callback invocation.
			diag_groups_holder: list[list[object] | None] = [None]

			class _GroupCapture:
				"""Capture group partition from the first round_callback."""
				...
				def capture(self, players: list[object]) -> list[list[object]] | None:
					if self._gs > 0 and self._gs < len(players):
						return _ppig(players, self._gs)
					return None

			group_capture = _GroupCapture(gs_int, diag_groups_holder)
			round_diagnostics = _L2RoundDiagnostics(
				local_group_size=gs_int, seed=int(seed), groups=None,
			)

			# Wrap callback to capture groups on first call
	...
	for combined_row in all_combined_rows:
		gs_val = int(combined_row["local_group_size"])
	...
	close_l2 = bool(active_rows) and all(str(row["hard_stop_fail"]) == "yes" for row in active_rows)
...
def main() -> None:
	parser = argparse.ArgumentParser(description="L2 local-group growth G2 scout harness")
	parser.add_argument("--seeds", type=str, default="45,47,49")
	parser.add_argument("--local-group-sizes", type=str, default="0,5,10,20,50")
	...
	result = run_l2_scout(
		...
		local_group_sizes=_parse_int_list(args.local_group_sizes),
	...
	print(f"close_l2={result['close_l2']}")
