"""A1 asynchronous replicator – G2 scout harness.

Follows SDD §2.7 A1 protocol lock.  CLI-compatible with the B-series harness pattern.
No G1 deterministic gate (mean-field has no players; async is N/A).
"""
...
from evolution.replicator_dynamics import async_replicator_step, sampled_growth_vector
...
DEFAULT_OUT_ROOT = REPO_ROOT / "outputs" / "a1_async"
DEFAULT_SUMMARY_TSV = REPO_ROOT / "outputs" / "a1_async_summary.tsv"
DEFAULT_COMBINED_TSV = REPO_ROOT / "outputs" / "a1_async_combined.tsv"
DEFAULT_DECISION_MD = REPO_ROOT / "outputs" / "a1_async_decision.md"

GATE = "g2_sampled"

SUMMARY_FIELDNAMES = [
	"condition",
	"async_update_fraction",
	"seed",
	"cycle_level",
	"stage3_score",
	"turn_strength",
...
def _condition_name(*, frac: float) -> str:
	token = f"{float(frac):.2f}".replace(".", "p")
	return f"{GATE}_frac{token}"


def _control_condition_name() -> str:
	return _condition_name(frac=1.0)
...
class _A1RoundDiagnostics:
	"""Collect per-round async diagnostics during simulate() via round_callback."""

	def __init__(self, *, frac: float, seed: int) -> None:
	...
	def callback(
		...
		# Weight dispersion across players
		strategy_space = ["aggressive", "defensive", "balanced"]
		n_total = len(players) if players else 1
		per_strategy_std: list[float] = []
		for s in strategy_space:
			vals = [float(getattr(pl, "strategy_weights", {}).get(s, 1.0)) for pl in players]
			m = sum(vals) / float(len(vals)) if vals else 1.0
			var = sum((v - m) ** 2 for v in vals) / float(len(vals)) if vals else 0.0
			per_strategy_std.append(var ** 0.5)
		self.records.append({
			"weight_dispersion": _safe_mean(per_strategy_std),
		})
	...
	def finalize(self, *, rows: list[dict[str, Any]], burn_in: int, tail: int) -> dict[str, Any]:
		...
		return {
			"mean_fraction_updated": float(self.frac),
...
def _write_simplex_plot(
	...
	*,
	...
	try:
		...
		raise RuntimeError("matplotlib is required to write A1 simplex plots") from exc
...
def _write_phase_amplitude_plot(
	...
	*,
	...
	try:
		...
		raise RuntimeError("matplotlib is required to write A1 phase-amplitude plots") from exc
...
def _build_condition_summary(
	*,
	...
	return {
		"condition": condition,
		"async_update_fraction": _format_float(frac),
		"is_control": _yes_no(float(frac) >= 1.0),
		...
		"gamma_uplift_ratio_vs_control": "",
		"stage3_uplift_vs_control": "",
		"mean_fraction_updated": _format_float(_safe_mean([float(r["mean_fraction_updated"]) for r in per_seed_rows])),
		...
		"verdict": "control" if float(frac) >= 1.0 else "pending",
		"representative_seed": "",
		"representative_simplex_png": "",
		"representative_phase_amplitude_png": "",
...
def _representative_seed_row(rows: list[dict[str, Any]]) -> dict[str, Any]:
	return max(
		...
		key=lambda row: (
			...
			float(row["mean_weight_dispersion"]),
			-int(row["seed"]),
		),
	)
...
def _write_decision(
	...
	*,
	...
	lines = [
		"# A1 Asynchronous Replicator Decision",
		"",
	...
	for row in combined_rows:
		lines.append(
			f"- {row['condition']}: frac={row['async_update_fraction']}"
	...
	if not recommended_candidates:
		...
		for row in recommended_candidates:
			lines.append(
				...
				f" frac={row['async_update_fraction']}"
				f" stage3_uplift={row['stage3_uplift_vs_control']}"
				f" mean_weight_dispersion={row['mean_weight_dispersion']}"
				f" representative_seed={row['representative_seed']}"
			)
	lines.extend(["", "## Stop Rule", ""])
	lines.append("- G2: 若 async_update_fraction <= 0.5 的 active cells 全部 0/3 Level 3 且 mean_stage3_score uplift < 0.02，A1 直接 closure。")
	if close_a1:
		lines.append("- overall_verdict: close_a1")
	else:
		lines.append("- overall_verdict: keep_a1_open")
	path.parent.mkdir(parents=True, exist_ok=True)
	path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run_a1_scout(
	*,
	...
	if 1.0 not in {float(f) for f in async_fractions}:
		raise ValueError("A1 short scout requires async_fractions to include 1.0 for matched controls")
	...
	for frac in async_fractions:
		frac_f = float(frac)
		condition = _condition_name(frac=frac_f)
		...
		for seed in seeds:
			cfg = SimConfig(
				...
				memory_kernel=int(memory_kernel),
				async_update_fraction=frac_f,
			)
			round_diagnostics = _A1RoundDiagnostics(frac=frac_f, seed=int(seed))
			...
			provenance = {
				...
				"round_diagnostics": {
					"mean_fraction_updated": float(diag_stats["mean_fraction_updated"]),
			...
			per_seed_rows.append({
				"condition": condition,
				"async_update_fraction": _format_float(frac_f),
				"seed": int(seed),
				...
				"mean_fraction_updated": _format_float(diag_stats["mean_fraction_updated"]),
	...
	for combined_row in all_combined_rows:
		frac_val = float(combined_row["async_update_fraction"])
		...
		combined_row["gamma_uplift_ratio_vs_control"] = _format_ratio(1.0 if frac_val >= 1.0 else gamma_ratio)
		combined_row["stage3_uplift_vs_control"] = _format_float(0.0 if frac_val >= 1.0 else stage3_uplift)

		is_control = frac_val >= 1.0
		short_scout_pass = (not is_control) and level3_seed_count >= 1 and stage3_uplift >= 0.02
		hard_stop_fail = (
			not is_control
			and frac_val <= 0.5 + 1e-12
	...
	for condition, per_seed_rows in condition_seed_rows.items():
		for row in per_seed_rows:
			...
			if str(row["condition"]) == control_cond:
	...
	active_rows = [
		...
		if str(row["is_control"]) == "no" and float(row["async_update_fraction"]) <= 0.5 + 1e-12
	]
	close_a1 = bool(active_rows) and all(str(row["hard_stop_fail"]) == "yes" for row in active_rows)
...
def main() -> None:
	parser = argparse.ArgumentParser(description="A1 asynchronous replicator G2 scout harness")
	parser.add_argument("--seeds", type=str, default="45,47,49")
	parser.add_argument("--async-fractions", type=str, default="0.1,0.2,0.5,1.0")
	...
	result = run_a1_scout(
		...
		async_fractions=_parse_float_list(args.async_fractions),
	...
	print(f"close_a1={result['close_a1']}")
