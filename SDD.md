...
- [環境設定：Ollama（WSL2 → Windows）](#環境設定ollamawsl2--windows)
  - [前置（Windows，只需做一次）](#前置windows只需做一次)
  - [WSL2 環境變數（已寫入 `~/.bashrc`，自動生效）](#wsl2-環境變數已寫入-bashrc自動生效)
  - [測試連線](#測試連線)
  - [Python 呼叫範例](#python-呼叫範例)
- [環境設定：RL Session API（Godot → WSL2）](#環境設定rl-session-apigodot--wsl2)
  - [目前可用的連線位址](#目前可用的連線位址)
  - [連線原則](#連線原則)
- [0) 本 repo 的分層（當作 SDD 的約束）](#0-本-repo-的分層當作-sdd-的約束)
- [1) 研究目標（Spec 的最高層）](#1-研究目標spec-的最高層)
  - [1.1 研究主目標（方案 B）](#11-研究主目標方案-b)
  - [1.2 非目標（避免範圍膨脹）](#12-非目標避免範圍膨脹)
  - [1.3 Spec 版本與變更規則（研究型 SDD 的「鎖死點」）](#13-spec-版本與變更規則研究型-sdd-的鎖死點)
- [2) 數學規格（可執行的形式化定義）](#2-數學規格可執行的形式化定義)
  - [2.1 策略比例向量（simplex 狀態）](#21-策略比例向量simplex-狀態)
  - [2.2 Payoff 規格（兩種 mode）](#22-payoff-規格兩種-mode)
  - [2.4 Dungeon 自適應制衡（Adaptive Counter；可選功能）](#24-dungeon-自適應制衡adaptive-counter可選功能)
  - [2.4 時間索引（非常重要：避免「看起來對」但其實錯位）](#24-時間索引非常重要避免看起來對但其實錯位)
  - [2.5 新 payoff / memory / evolution 擴充規格（H-series）](#25-新-payoff--memory--evolution-擴充規格h-series)
  - [2.5.1 研究 family 切分（落地版）](#251-研究-family-切分落地版)
  - [2.6 B-series：sampled-side geometry preservation（2026-04-03, updated 2026-04-07）](#26-b-seriessampled-side-geometry-preservation2026-04-03-updated-2026-04-07)
- [3) 不變條件（Invariants）](#3-不變條件invariants)
  - [3.1 數值不變條件](#31-數值不變條件)
  - [3.1A Event line invariants（W4 / restart guard）](#31a-event-line-invariantsw4--restart-guard)
  - [3.1B World feedback mode invariants（Phase coupling）](#31b-world-feedback-mode-invariantsphase-coupling)
  - [3.2 分層不變條件（架構約束）](#32-分層不變條件架構約束)
  - [3.3 資料契約（CSV schema）](#33-資料契約csv-schema)
  - [3.4 指標語意（避免 reward/utility 混用）](#34-指標語意避免-rewardutility-混用)
- [4) 驗證標準（Verification / Definition of Done）](#4-驗證標準verification--definition-of-done)
  - [4.1 最低驗收（MVP 研究版）](#41-最低驗收mvp-研究版)
  - [4.1.1 參數介面（以現行實作為準）](#411-參數介面以現行實作為準)
  - [4.2 循環動態的可觀測驗收（建議指標）](#42-循環動態的可觀測驗收建議指標)
  - [4.3 正式分級判定（建議採用：cycle_level = 0/1/2/3）](#43-正式分級判定建議採用cycle_level--0123)
  - [4.4 控制組定義（False Positive Guard；建議列為硬性驗收）](#44-控制組定義false-positive-guard建議列為硬性驗收)
  - [4.5 研究級穩健性需求（Robustness Requirements；建議用於發表）](#45-研究級穩健性需求robustness-requirements建議用於發表)
  - [4.6 有限尺寸臨界分析（Finite-size critical transition；下一階段主線）](#46-有限尺寸臨界分析finite-size-critical-transition下一階段主線)
  - [4.7 發表級加嚴待辦（Parking lot：先記著，等穩定區域找到再做）](#47-發表級加嚴待辦parking-lot先記著等穩定區域找到再做)
- [5) 邊界條件（Boundary conditions）](#5-邊界條件boundary-conditions)
- [6) 測試設計（Test design）](#6-測試設計test-design)
  - [6.1 單元測試（純函數/不含隨機）](#61-單元測試純函數不含隨機)
  - [6.2 契約測試（CSV schema）](#62-契約測試csv-schema)
  - [6.3 隨機性控制（可選，但非常建議）](#63-隨機性控制可選但非常建議)
- [7) 模組規劃（把 Spec 映射到程式結構）](#7-模組規劃把-spec-映射到程式結構)
  - [7.1 研究主線（循環動態）最小閉環](#71-研究主線循環動態最小閉環)
  - [7.4 文本 → 9 維人格推斷（LLM 路徑；方案 A）](#74-文本--9-維人格推斷llm-路徑方案-a)
  - [7.5 文本 → 9 維人格推斷（SBERT 路徑；方案 B）](#75-文本--9-維人格推斷sbert-路徑方案-b)
  - [EXP-1.2：逆反期機制（設計草案）](#exp-12逆反期機制設計草案)
  - [7.4 B-series 主線契約（2026-04-03, updated 2026-04-07）](#74-b-series-主線契約2026-04-03-updated-2026-04-07)
  - [§(E1) Heterogeneous RL — 異質 α/β per player](#e1-heterogeneous-rl--異質-αβ-per-player)
  - [§(F1) Heterogeneous Payoff — per-player payoff perturbation](#f1-heterogeneous-payoff--per-player-payoff-perturbation)
  - [§(E2) Lower β Ceiling — 保留 E1 wide α + 降低 β](#e2-lower-β-ceiling--保留-e1-wide-α--降低-β)
  - [§(CC1) Classifier Calibration — Stage 2 Phase Rotation R² Fallback](#cc1-classifier-calibration--stage-2-phase-rotation-r²-fallback)
  - [§(G1) Entropy Regularization — Q-update 層 Entropy Bonus](#g1-entropy-regularization--q-update-層-entropy-bonus)
  - [§(F1v2) Payoff Perturbation v2 — E2-best base + mild ε](#f1v2-payoff-perturbation-v2--e2-best-base--mild-ε)
  - [§(BL1) Baseline Lock — well_mixed ε=0.02](#bl1-baseline-lock--well_mixed-ε002)
  - [§(L3-OPT) Level 3 優化時代 — 實驗路線圖](#l3-opt-level-3-優化時代--實驗路線圖)
- [8) 實作步驟（SDD 迭代節奏）](#8-實作步驟sdd-迭代節奏)
- [9) Review checklist（每次改動前先問自己）](#9-review-checklist每次改動前先問自己)
- [10) 附錄：本輪決策快照（2026-04-13）](#10-附錄本輪決策快照2026-04-13)
  - [10.1 本輪範圍與結論](#101-本輪範圍與結論)
  - [10.2 W1~W4 完成快照](#102-w1w4-完成快照)
  - [10.3 本輪量化快照](#103-本輪量化快照)
  - [10.4 驗收清單快照（W4 closure）](#104-驗收清單快照w4-closure)
  - [10.5 守門條件維持](#105-守門條件維持)
- [11) 理論驗證階段封板快照（2026-05-18）](#11-理論驗證階段封板快照2026-05-18)
  - [11.1 三大管線封板結論](#111-三大管線封板結論)
  - [11.2 BL2 數值底牌（Runtime Bridge Anchor）](#112-bl2-數值底牌runtime-bridge-anchor)
  - [11.3 Track A 9D Baseline 鎖定值（2026-05-18 更新）](#113-track-a-9d-baseline-鎖定值2026-05-18-更新)
  - [11.4 技術債記錄（延期至 Track B）](#114-技術債記錄延期至-track-b)
  - [11.5 下一階段進入條件（Runtime Bridge）](#115-下一階段進入條件runtime-bridge)
- [12) Runtime Bridge v2：Godot 前端整合架構規格（2026-05-18 起稿）](#12-runtime-bridge-v2godot-前端整合架構規格2026-05-18-起稿)
  - [12.1 分層與職責劃分（新增 Runtime Bridge 層）](#121-分層與職責劃分新增-runtime-bridge-層)
  - [12.2 核心資料結構：RLSessionEngine 介面簽章](#122-核心資料結構rlsessionengine-介面簽章)
  - [12.3 FrameSnapshot 結構（ResponseEnvelope 擴充）](#123-framesnapshot-結構responseenvelope-擴充)
  - [12.4 Session 生命週期與非同步 burn-in](#124-session-生命週期與非同步-burn-in)
  - [12.5 Cycle Metrics 即時檢測（sliding-window 輕量版）](#125-cycle-metrics-即時檢測sliding-window-輕量版)
  - [12.6 輸出契約（向後相容性）](#126-輸出契約向後相容性)
  - [12.7 實作順序與驗收標準](#127-實作順序與驗收標準)
  - [12.8 不得做的事（禁止區）](#128-不得做的事禁止區)
  - [12.9 技術債與 Track B 待辦](#129-技術債與-track-b-待辦)
<!-- TOC END -->

## 環境設定：Ollama（WSL2 → Windows）

> Ollama 安裝在 Windows，WSL2 透過 Host IP 橋接。

### 前置（Windows，只需做一次）

```powershell
# PowerShell：讓 Ollama 監聽所有介面
[System.Environment]::SetEnvironmentVariable("OLLAMA_HOST", "0.0.0.0:11434", "User")
# 之後重啟 Ollama（工作列圖示右鍵 → Quit → 重新開啟）
```

### WSL2 環境變數（已寫入 `~/.bashrc`，自動生效）
...
- `players/`：玩家抽樣策略 + 人格模型（9D Enneagram；trait set 已鎖定）
...
- 不是要先做完整 UI / 內容型地下城關卡
- 不是要一次做完人格系統 + 事件系統 + Boss stackelberg
...
### 2.4 Dungeon 自適應制衡（Adaptive Counter；可選功能）

#### 概念（源自 World Framework §5 Meta Layer / Little Dragon）

地下城觀察當前族群策略分布，當某策略明顯主導時，自動調整事件出現頻率與風險，使主導策略承受更大壓力（非零和制衡）。

#### 實作位置

`dungeon/dungeon_ai.py`：`DungeonAI.adapt_to_dominant()`遺言的累積效應（W2.1 實驗結果）：
多 life 跑下來，人格分布明顯朝 endurance / stability_seeking / risk_aversion 收斂——testament 主要放大保守存活策略，而非創造新的旋轉結構（這也是 W2.1 最終封卷的原因）。

需搭配 `event_loader` 才有效果；預設關閉（`adaptive_counter=False`）。

#### 參數契約

| 參數 | 型別 | 預設 | 說明 |
|---|---|---|---|
| `adaptive_counter` | bool | False | 啟用制衡機制（需 event_loader） |
| `counter_strength` | float | 0.3 | 制衡強度（0 = 無效果，1 = 最大） |
| `counter_threshold` | float | 0.40 | 觸發閾值：dominant 比例需超過此值才制衡 |

#### 策略 → 事件映射

| 主導策略 | 承壓事件（風險 ×） | 劣勢事件（出現率 ↑） |
|---|---|---|
| `aggressive` | Threat（攻擊風險升）| Uncertainty × 1.5、Navigation × 1.3 |
| `defensive` | Resource（資源爭奪升）| Threat × 1.5、Uncertainty × 1.3 |
| `balanced` | Uncertainty（探索風險升）| Resource × 1.5、Threat × 1.3 |

#### 行為契約（Invariants）

1. `adaptive_counter=False` 時，`set_popularity()` 行為必須完全退化回舊版（不呼叫 `adapt_to_dominant()`）。
2. `event_loader is None` 時，`adapt_to_dominant()` 直接回傳 `None`，不拋出例外。
3. dominant 比例 < `counter_threshold` 時，重置為全 1.0 中性權重（避免殘留效果）。
4. risk multiplier 強制 clamp 至 `[0.5, 2.0]`（由 `EventLoader._resolve_family_multipliers` 保證）。
5. 調整量以 `intensity = counter_strength × excess` 線性縮放，其中 `excess = (p_dominant - counter_threshold) / (1 - counter_threshold)`。
...
#### 實體註冊表匯出契約：`outputs/registry_manifest.json`

目的：將 `outputs/personality_frontier_v1.json` 壓縮成 SQLite 友好的扁平實體註冊表。此匯出只保留每個 selected frontier entity 的 final snapshot，不得保留 `axis_values`、`re_tail`、`z_history` 這類完整軌跡。唯一合法匯出器為 `scripts/finalize_registry.py`。
...
- `formula_version`: 固定為 `v2_log_e_k`
- `python_version`: 目前 venv 版本（本 repo 以 3.10.12 為準）
- `source_frontier_path`: 來源 frontier JSON
- `source_summary_path`: 原始 NSGA summary
- `source_hash`: 來源 frontier JSON 的 `sha256_*`
- `sample_count`: `entities` 筆數
- `selection`: `{method, trait_precision, raw_candidates, unique_candidates, selected_candidates}`
- `signal_basis`: `expanding/contracting/exploring` 的 projection basis，其中 exploring 需保留 ASY weights
- `source_scan`: 來源 frontier 的 scan 參數，至少包含 `scan_min`, `scan_max`, `scan_step`, `t_steps`, `tail_start`, `peak_fraction`, `width_rule`, `gap_tolerance`, `resonance_max_lag`, `fitness_k`
- `tag_thresholds`: 由 frontier 的 `thresholds` 繼承，用於重建 archetype tags
- `norm_basis`: `re_peak` / `sw_width` / `z_drift` 的 min-max basis
- `width_stats`: `sw_width_zero_count`, `sw_width_positive_count`, `sw_width_hit_rate`，用於區分「零寬度」是 censor 還是觀測到的真零
- `sw_width`: peak-relative width，預設以 `re_peak × peak_fraction` 為門檻，保留 3-point trailing moving average 與 `gap_tolerance`，不再依賴絕對 RE 閾值

`entities` 每筆必須為扁平物件，欄位固定為：
...
- `entity_id = ent_s{seed}_t{trial_id}`（deterministic slug，非 UUID 亦可）
- `rank` 使用來源 `frontier_rank`
- `w_exp/w_con/w_ext` 對應來源 `signal_snapshot` 的 `z_expanding/z_contracting/z_exploring`
- `re_peak = asy.metrics.peak_re`
- `sw_width = asy.metrics.sweetspot_width_axis`
- `z_drift = asy.metrics.drift_median`
- `slr = candidate.slr`

Normalization 規則：

- `n_x = 0.0` if `max_x == min_x` else `(x - min_x) / (max_x - min_x)`
- `n_energy` 由 `re_peak` 正規化
- `n_stability` 由 `sw_width` 正規化
- `n_drift` 由 `z_drift` 正規化

Tag 規則：

- `tag_survivor = 1` iff `[The Survivor]` 存在，或對應 `sw_width >= width_p90` 且 `drift_p95 <= drift_p50`
- `tag_pulsar = 1` iff `[The Pulsar]` 存在，或對應 `re_peak >= peak_p90` 且 `resonance_mean >= resonance_p50`
- `tag_alpha = 1` iff `tag_survivor = 1` 且 `tag_pulsar = 1`

不變條件：

- 不得輸出完整軌跡或深層 nested entity schema
- 不得在匯出器內重跑掃描或重新模擬
- `entities` 必須只保留基礎型別欄位（int / float / string / list of strings）
- 若輸入 frontier 空缺或欄位不完整，必須直接 fail，不得以預設值補齊
...
### 2.5.1 研究 family 切分（落地版）

本節把現階段評估整理成可直接落地的規格邊界。判斷順序固定如下：

1. 是否仍維持三策略 `aggressive/defensive/balanced` 的 simplex 主線
2. 是否仍可沿用既有 `simulation -> CSV -> analysis` 契約
3. 是否仍可用現有 `cycle_level / env_gamma / theory anchors` 做驗收

只要任一項不成立，就不應再塞回現有主線，而是另開 family。

#### A. 可沿用現有主線

這一類變更的共同條件是：仍在三策略 simplex 內、仍可用既有 CSV schema、仍可由現有 cycle metrics 驗證，而且必須保留 no-op 退化等價性。

- `H1`：`memory_kernel` 的記憶窗變化（`1/3/5`）
- `H2`：`threshold_ab`、hysteresis band、trigger / state smoothing
- `H3`：`hetero`、`fixed_subgroup_share`、`fixed_subgroup_weights`、`anchor_pull`、`coupling`、`bidirectional coupling`、`state-dependent coupling`
- `H4`：`hybrid_update_share` 與 `hybrid_inertia`
- `H5`：`sampled_inertia`
- `B4 / B3 / B5` 類 sampled-side geometry preservation、stratified growth、tangential drift

落地規格：

- 所有變體都要能在 `selection_strength / memory_kernel / threshold / subgroup / inertia` 的 no-op 點精確退回基線
- 主要驗收仍用既有工具：`simulation.run_simulation`、`simulation.seed_stability`、`simulation.rho_curve`、`simulation.grid_refine`、`analysis.cycle_metrics`、`analysis.theory_anchors`
- 若只是在現有主線內做機制微調，輸出仍只使用現有 CSV 契約，不新增主欄位
- 這一類修正的成功標準，不是「一定要出 Level 3」，而是「在既有 plateau 內是否能穩定改變 seed-level 或 aggregate 指標，且 no-op 必須守住」

#### B. 必須另開新 family

只要下面任一條成立，就不應再沿用現有主線的 3-strategy / Stage1-3 評估框架：

- `strategy_space` 不是 3，而是 5、7、11 或任意可變長度
- payoff 不再是 pairwise matrix，而是明確的 HOI tensor / 多方交互
- 驗收目標改成 chaos、strange attractor、Lyapunov exponent、delay embedding、Poincaré 類指標
- 需要把 simplex normalization、phase metrics、CSV schema 一起升級到 N 維泛化

新 family 的落地規格：

- 先寫新 Spec，再寫碼；不得直接借用現有 Level 0-3 分級當主驗收
- 必須先定義新的 `strategy_space` contract、normalization 規則、time index、以及至少一組 family-specific 指標
- 現有 `Level 0-3` 只可當 sanity check 或 regression baseline，不可直接當新 family 的主要 success criterion
- 任何 HOI / chaos / odd-species 實驗，都要先補新的測試矩陣，再進 simulation sweep

最小測試矩陣（建議落地順序）：

1. 現有主線 no-op 回歸：`matrix_ab`、`threshold_ab(a_hi=a,b_hi=b)`、`hetero(k_A=k_D=k_B)`、`fixed_subgroup_share=0`、`sampled_inertia=0`
2. 現有主線增量回歸：memory / threshold / subgroup / hybrid / inertia / tangential drift 的單點 smoke
3. 新 family 基線：`N=5` 或 `N=7` 的奇數策略循環，先驗證新 normalization 與新 time index
4. 新 family 進階：HOI tensor 或 chaos 指標，只在第 3 步通過後再開

這個切分的實際含義是：

- 現有主線可以繼續做，但只能在既有三策略架構內前進
- 奇數物種、混沌吸引子、HOI 張量屬於新的研究家族，不能再當作現有主線的局部 tuning
...
每位玩家 $p$ 的 personality-driven 信號（9D Enneagram；`personality_coupling.personality_signal_k / _mu`）：

$$
z_k^{(p)} = assertiveness^{(p)} - risk\_aversion^{(p)}
$$

$$
z_\mu^{(p)} = 0.5\,(stability\_seeking^{(p)} + endurance^{(p)}) - 0.5\,impulsiveness^{(p)}
$$
...
per-player inertia 定義為 H7.3 paradigm shift 後的「慣性調節器」版本（取代舊「噪聲」觀點）：
...
**範式說明**（H7.3 μ-梯度掃描後確認）：
- 舊觀點：μ_base 是「相位噪聲基準」，人格為擾動
- **新觀點**：μ_base 是「相位累積容量」（bifurcation parameter），人格在此基準周圍調節個體記憶能力
- 物理意義：μ 決定速度向量記憶的半衰期；$\tau_{1/2} \approx \ln(2) / \ln(1/\mu)$
- 三區域分布：$\mu \le 0.20$ （無效區：人格無差異）→ $\mu \approx 0.30$ （共鳴區：人格決定性調節）→ $\mu \ge 0.35$ （陷阱區：L0 lock-in 主導）

不變條件（H7.3 後更新）：

- `beta_state_k = 0` 時，B4 必須精確退化回既有 H7.1 `personality_coupled` 路徑
- `lambda_k = 0`, `lambda_mu = 0`, `mu_base = 0.30`, `beta_state_k = 0` 時，必須精確退化回 H7.3-validated 黃金工作點
...
第一版 personality signal 固定使用（9D Enneagram；`personality_coupling.personality_signal_k`）：
...
### 3.1A Event line invariants（W4 / restart guard）

本小節鎖定 `simulation.personality_rl_runtime` 與 `simulation.b1_async_dispatch_gate` 的事件線 invariant。

- E6：`event_neutralize_payoff`
  - 契約：對每輪受影響玩家集合 $S_t$，若啟用 neutralize，則事件 reward 偏移需滿足

$$
\frac{1}{|S_t|}\sum_{i\in S_t}\Delta u_i(t)=0
$$

  - 工程判據：診斷欄位 `event_neutrality_max_abs_mean` 必須 `<= event_neutralize_eps`；否則 `event_neutrality_pass=false`
  - 退化條件：`event_neutralize_payoff=false` 時，必須退化回既有事件注入路徑，不改動原 reward 注入邏輯

- E8：`event_trigger_mode=entropy_guard`
  - 契約：只有當 normalized entropy $H_t$ 高於閾值 `event_trigger_entropy_threshold` 時，事件注入才允許啟動
  - `event_trigger_mode=always` 時，必須退化回既有無條件注入路徑
  - 診斷契約：必須輸出 `event_trigger_guard_check_count`, `event_trigger_guard_block_count`, `event_trigger_guard_block_rate`, `event_trigger_guard_pass`

- B1 gate invariant 契約（summary JSON）
  - `gate.invariant_neutrality_pass`
  - `gate.invariant_trigger_guard_pass`
  - `gate.invariant_overall_pass = invariant_neutrality_pass AND invariant_trigger_guard_pass`
  - `gate.overall_pass` 必須包含 `invariant_overall_pass`（不可只看 L1/healthy/fairness/new_l1）

### 3.1B World feedback mode invariants（Phase coupling）

本小節鎖定 `simulation.personality_rl_runtime` 與 `simulation.b1_async_dispatch_gate` 的 world feedback mode 契約。

- `world_feedback_mode` 枚舉（固定）：
  - `off`
  - `adaptive_world`
  - `read_only`
  - `difficulty_only`

- 退化條件（backward compatibility）：
  - 若 `world_feedback_mode=off`，則 world channels 必須退化為不影響系統動力學（既有路徑）
  - 若舊旗標 `world_feedback=true` 且 `world_feedback_mode` 未明示，必須退化映射到 `adaptive_world`

- Phase 1（`read_only`）契約：
  - 允許 world state 更新與輸出（只觀測、可記錄）
  - 不允許 world state 影響事件抽樣、reward、risk、policy
  - 工程判據：`readonly_leak_score <= 1e-6`
  - 工程判據：`world_readonly_applied=1`（啟用 `read_only` 的 run）

- Phase 2（`difficulty_only`）契約：
  - 允許 world state 影響事件抽樣難度（sampling weights / difficulty index）
  - 不允許 world state 影響 reward、risk、policy payoff 幾何
  - 工程判據：`payoff_static_pass=true`
  - 工程判據：`difficulty_modulation_applied=1`（啟用 `difficulty_only` 的 run）

- CSV 契約（新增欄位，simulation 層維護）：
  - `world_state_scarcity`
  - `world_state_threat`
  - `world_state_noise`
  - `world_state_intel`
  - `world_readonly_applied`
  - `readonly_leak_score`
  - `difficulty_index`
  - `event_difficulty_multiplier`
  - `difficulty_modulation_applied`
  - `payoff_static_pass`

- 診斷/摘要契約：
  - runtime diagnostics/provenance 必填：
    - `world_feedback_mode`
    - `world_readonly_applied`
    - `readonly_leak_score`
    - `readonly_leak_pass`
    ...
    - `payoff_static_score`
    - `payoff_static_pass`
  - B1 gate summary JSON 必填：
    - `gate.invariant_readonly_leak_pass`
    - `gate.invariant_payoff_static_pass`
    - `gate.invariant_overall_pass` 必須包含 `invariant_readonly_leak_pass`
    - `gate.invariant_overall_pass` 必須包含 `invariant_payoff_static_pass`

- analysis 契約：
  - `analysis/event_provenance_summary.py` 新增 `compute_world_readonly_leak`
  - `analysis/event_provenance_summary.py` 新增 `verify_payoff_static`
  - `analysis/event_provenance_summary.py` 新增 `summarize_difficulty_modulation`
  - 該函式只能依 CSV/JSON 輸入計算，不得 import simulation
...
| 最大輸入長度 | 20 字元（`PERSONALITY_TEXT_MAX_LEN`） | 20 字元 |
...
驗證 W2.1 的跨 life 保守收斂，是否主要由 `Delta_dom` 的正回饋所驅動；若把 dominant strategy template 的符號翻轉，系統是否會從「越用越保守」改為「越用越逆反」的非線性跨代漂移。

#### 操控範圍

- 只修改 testament 中的 `Delta_dom_i` 符號，不改 `Delta_util_i`、`Delta_event_i`、`alpha_testament`、死亡閾值或 `risk` 契約。
- 以目前 `w2_episode.py` 的 9D `DOMINANT_TEMPLATES` 為準：`impulsiveness / assertiveness / optimism`、`risk_aversion / suspicion / endurance`、`curiosity / randomness / stability_seeking`。
- 逆反期版本定義為：`Delta_dom_i^rev = - Delta_dom_i`，其他項維持不變。

#### 候選 cell

| cell | testimonial sign | `alpha_testament` | 用途 |
|---|---|---:|---|
| control | positive | 0.00 | 單 life 退化基線，確認沒有 testament 時的漂移形狀 |
| w2_base | positive | 0.12 | 現行 W2.1 參考基準 |
| w2_strong | positive | 0.22 | 強版正回饋參考 |
| exp1_2_rev_base | reverse | 0.12 | 逆反期主 cell，測試負回饋是否解除保守收斂 |
| exp1_2_rev_strong | reverse | 0.22 | 逆反期強度版，測試是否出現過度擺盪或反向鎖定 |

#### 主要觀測

1. `tail_mean_personality_centroid_json` 是否仍朝 `caution / stability_seeking / endurance` 收斂，或改為反向漂移。
2. `tail dominant strategy flip rate` 是否高於正回饋基準，代表生命間風格切換被放大。
3. `tail mean personality autocorr` 是否下降，或在 lag 1 / 2 出現負相關，代表逆反期效應。
4. `life 4..6` 的 `mean_stage3_score` 與 `level3_seed_count` 是否仍可維持，不至於把系統直接打成單純崩塌。
5. `tail_dominant_flip_rate` 需寫入 combined summary：以 tail lives 的 `dominant_strategy_last500` 計算相鄰 life 翻轉率（翻轉次數 / (有效 life 數 - 1)）。

#### 預期 gate 與收官條件

- **Pass**：至少 1 個 reverse cell 在 `life 4..6` 內相較 control 出現更高的 dominant flip rate，且保留可觀測的 Level 3 seed。
- **Structural pass**：若 `exp1_2_rev_base` 未通過、但 `exp1_2_rev_strong` 通過，仍視為結構性 Pass。這代表逆反期效應存在臨界閾值，必須加大 testament 藥量，才能同時對抗 50% 績效分與環境死亡壓力；此時不得降級為 Weak positive。
- **Weak positive**：reverse cell 明顯降低保守 centroid drift，但 Level 3 只維持在邊界狀態。
- **Fail**：reverse cell 只造成更快崩塌或更單調的保守鎖定，沒有任何逆反期訊號。
- **Closure**：若 reverse sign 仍無法打破 W2.1 的保守收斂引力，則直接封檔，不再做 alpha_testament 細掃，轉向其他機制主線。
...
W3.4：9-persona robustness 判決（Action A/B closure；2026-05-11）

74. 本節鎖定 `w3_commit_push`（W3.1）與 `w3_policy_crossguard`（W3.2）在 9-persona heterogeneous setting 下的 robustness 判決；其目的不是追求更高峰值 `s3_score`，而是驗證「是否可同時維持 L3 啟動與長壽命相干」
75. 第一輪判決 protocol 鎖定為：`players=300`, `rounds=6000`, `selection_strength=0.10`, `memory_kernel=3`, `init_bias=0.12`, `events_json=DEFAULT_FULL_EVENTS_JSON`，短窗判讀固定 `burn=1000, tail=1000`，長窗判讀固定 `burn=2000, tail=2000`，dropout 探測固定 rolling-window（window=1000, step=20）
76. personality ablation 契約固定為：每輪對所有玩家同步縮放 `randomness`, `stability_seeking`, `curiosity` 三項 trait（scale 因子乘法）；其餘動力學與 payoff 幾何不得改動。此實驗只允許被解讀為「啟動門檻/壽命邊界」診斷，不得直接宣稱結構穩定化
77. 2026-05-11 第一輪結果正式鎖定為 negative result：在 seed=91 的窄幅掃描 `scale in {0.65,0.75,0.85,0.95,1.00}` 中，`scale<=0.95` 皆無法維持短窗 L3（皆記為 `L3_SUPPRESSED_EARLY`），僅 `scale=1.00` 觀測到短窗 L3 並於約 `1820/1840` rounds dropout
78. 12-seed 群體驗證（seeds=`{45,47,49,51,53,55,91,93,95,97,99,101}`）在 `scale=0.85` 與 `scale=1.00` 比較的 aggregate 判決固定為：`L3_SUPPRESSED_EARLY=7/12`, `DROPOUT_INTRODUCED=1/12`, `NO_DROPOUT_BOTH=4/12`（兩模組一致）；不得解讀為「dropout wall 後移成功」
79. W3 robustness 的研究解讀正式固定為：`w3_commit_push` / `w3_policy_crossguard` 在 9-persona 下屬於「條件相干（conditional coherence）的亞穩態短期共振」，其原生性質為「啟動硬門檻 + 啟動後有限壽命」；在本 protocol 下可採用工程近似 `L3_half_life ≈ 1800 +/- O(10^2)` rounds
80. 應用限制（硬規則）：若任務需求為 `>2000` rounds 的穩定吸引子，禁止把 `w3_commit_push` 或 `w3_policy_crossguard` 單獨視為可交付機制；後續若續做 W3，只允許走外部同步機制（例如 pulse policy class）並以 matched control 報告 uplift
81. W3.4 Option 2（Periodic pulse 2D scan；2026-05-12）判決固定為 negative result：在不破壞核心賽局拓樸之前提下，掃描 `periodic_interval in {180,240,300,360,480}` 與三檔 pulse 強度（low/medium/high）後，最佳 cell 僅達 `dropout_shift_vs_control=+120`（`interval=240`, `high`），未跨越有效干預門檻 `+200`
82. W3.4 Option 2 的反模式觀測需固定納入解讀：`interval>=360` 且強脈衝可能與殘餘 L2 震盪形成破壞性干涉（例如 `interval=360`, `high` 觀測 `dropout_shift=-100`）；`interval=180` 且低強度脈衝會退化為高頻雜訊注入（觀測 `short_s3_mean` 顯著下滑）
83. W3.4 Option 2 的設計結論固定為：正式排除「單一 payoff scalar pulse 即可延長 B1/B2 亞穩態壽命」之工程路線；不得再以同類 periodic interval/強度細網格作為主線突破方案
84. W3 後續升級方向（硬約束）：若要追求 `>2000` rounds 穩定吸引子，只允許進入更根本干預層（例如 memory-kernel 結構或 signal-projection 拓樸），不得在既有 scalar pulse 家族內做強度加碼（ultra-high）或 burst 疊脈衝作為主線 deliverable

W3.5：Signal Projection derived state（Option 3；2026-05-12）

85. W3.5 的研究對象固定為 signal projection matrix 介入後形成的 derived state，正式標籤為 `PROJECTED_STABLE_L2`；此標籤只描述「外部投影誘導下的高相干亞穩態」，不得直接升格為 `Level 3` 或新的基態相
86. `PROJECTED_STABLE_L2` 的物理特徵固定為：相較 control 具有顯著延壽（在 6000-round confirm 視窗內可達 `dropout_shift_vs_control=+2400` 級），但 `Stage 3 Score` 長窗仍約束在 `0.48 ~ 0.51`，且 `short_l3_count=1`, `long_l3_count=0`，表示未跨越 L3 相變點
87. 本狀態的語義判定固定為：系統已脫離隨機漂移（L1）吸引力，並被鎖定在受控高能亞穩態（projected L2+）；但因未達 `short_s3_mean >= 0.55` 與 `short_l3_count >= 2`，故不得宣稱 `relight` 成功，也不得把 L3 門檻下調以包裝語意
88. 若後續要做 confirm，固定 protocol 為：`seeds={45,47,49,51,53,55,91,93,95,97,99,101}`、`rounds=10000`、僅測 top-3 projection cells（`proj_triad_rotate g=0.75 θ=0.02`、`proj_triad_soft g=0.50 θ=0.02`、`proj_triad_focus g=0.75 θ=0.02`）；confirm 的目的只在檢查生存曲線與 score 漂移是否顯示結構性穩態，不得作為調降 L3 門檻的依據
89. 2026-05-13 更新：10,000-round confirm 結果（scripts/run_option3_signal_projection_confirm.py）已完成，結果為 Negative：
  - control baseline: `dropout_median=4460`, `short_s3_mean=0.51902`。
  - 三個 top confirm cells (`proj_triad_rotate_g0.75_th0.02`, `proj_triad_soft_g0.50_th0.02`, `proj_triad_focus_g0.75_th0.02`) 在 10k 窗口內的 `dropout_shift_vs_control=+0`，`short_l3_count=0`。
  - 統合判讀：先前 6,000-round 的壽命 uplift 在 10k confirm 下未被穩健複現，`PROJECTED_STABLE_L2` 在更長時域下被判定為非穩健（ephemeral / windowed uplift），不得升格為可交付之機制。

90. 後續建議（策略轉向）：
  - 優先檢驗系統的記憶性（Memory Kernel）對抗 9-persona 瞬時雜訊的能力（擴展 `memory_kernel` 至 5、10 並做 sweep）。
  - 並行/或備選：做一個 `reduced-persona` 測試（把 9-persona 降維到 3-persona），確認 collapse 在哪個維度開始不可逆。
  - 保持 L3 門檻（0.55）不變；任何門檻調整需建立在跨 N 與長時域可重現的證據上。

W3.6：Reduced-Persona Dimension Collapse Test（Option 4；2026-05-13）

91. W3.6 的研究對象固定為 persona 維度與系統穩定性的直接相關性，標籤為 `REDUCED_3_CORE`；此測試旨在檢驗「人格多樣性是否為 B1/B2 坍塌的根本原因」。

92. `REDUCED_3_CORE` 的定義固定為：從 9 個 DEFAULT_PERSONALITY_KEYS（impulsiveness, assertiveness, optimism, risk_aversion, suspicion, endurance, randomness, stability_seeking, curiosity）中僅保留三個核心軸（assertiveness, stability_seeking, curiosity），其餘六個人格維度在 player initialization 時強制歸零；每個 player 的三核軸值在 [-0.4, 0.4] 區間內從 seed-derived RNG 均勻採樣。

93. 2026-05-13 執行結果（scripts/run_reduced_persona_scan.py 完成）：
  - 掃描參數：6 seeds × 6000 rounds，memory_kernel=3, players=300，control 與 reduced3 並行
  - 控制基線：dropout_median=4820, short_s3_mean=0.5334, short_l3_count=0, long_l3_count=0
  - 降維測試：dropout_median=4820, short_s3_mean=0.5334, short_l3_count=0, long_l3_count=0 (shift vs control: +0, +0.0000)
  - **關鍵發現：lifespan 與 stage3_score 與 control 無差異；人格維度縮減未產生任何可測量的穩定性改善。**

94. 時序與頻譜診斷（詳見 scripts/analyze_reduced_persona.py 輸出）：
  - 最長存活 seed（seed47, dropout=4820）與最短存活 seed（seed55, dropout=4600）差異為 220 rounds（相對差異 4.6%）
  - 頻譜分析（FFT）：兩組 seeds 的坍塌簽徵均為 NOISE BURST 模式（low-freq/high-freq ratio ≈ 0.337–0.340，高頻成分明顯）
  - **解讀**：最小生存差異 + 一致的頻譜特徵 → 坍塌機制為準決定論的（quasi-deterministic），不是個體隨機性主導。

95. 統合判讀（四個intervention 全景）：
  - Option2（scalar pulses）：lifespan shift +120 << +200 gate → 失敗
  - Option3（signal projection）：6k 短視窗 uplift +2400 → 10k confirm dropout_shift=0 → 非穩健
  - Option4（reduced-persona）：人格簡化 → dropout_shift=+0 → 人格維度非根本原因
  - **結論**：B1/B2 的坍塌根源不在於【事件注入】、【幾何修正】、或【維度雜訊】，而在於基礎 payoff 矩陣與離散時間 sampled replicator 的固有衝突。

96. L3 門檻與不變條件鎖定：
  - 保持 `short_s3_mean >= 0.55` 與 `short_l3_count >= 2` 硬門檻不變
  - 不得基於單一樣本或短視窗 uplift 調降門檻；任何門檻變更需建立在跨多輪、長時域、多機制的一致證據
  - 四個月 W3 實驗（W3.1–W3.6）共計 ≈300 runs，未發現可穩健複現的 L3 relight；此為強 negative result

97. 後續建議（策略終局）：
  - W3 系列正式 closure：輾轉測試 payoff 調整、leader policy、信號投影、人格簡化均未突破 L2 plateau；需轉向基礎層改造
  - A 線（記憶性深掃）：Memory Kernel sweep (mk ∈ {3,5,10})；若 mk=10 仍無突破，記憶機制也非根本解
  - B 線（拓撲重設）：考慮基數改造（players、族群結構）或新的 payoff paradigm（非二階矩陣形式）
  - 對標：目前已排除 ≈10 個 hypothesis；system capacity limit 可能為物理常數而非可調参數
...
3. B4 第一輪只允許掃描：`beta_state_k ∈ {0.0, 0.3, 0.6, 1.0}` 與 `k_base ∈ {0.06, 0.08}`；並固定 `lambda_k=0.0`, `lambda_mu=0.0`, `mu_base=0.0`（**H7.3 後更新**：若改為 mu_base=0.30 與 λ_μ=0.05，請參考 H7.3 黃金點規範與修訂 3-gate 框架）
...
**2026-05-17 修正紀錄**：`outputs/p21a_extended/p21a_decision.md` 原先因 gate 程式碼未套用 `|γ|<1e-4` 容許誤差，將 mild_cw 誤判為 `pass=no`。實際 per-seed env_gamma 均在 `[-2e-6, -1e-6]` 區間（mean = -1.17e-6），與 BL1 通過條件的 `max_abs = 1e-6` 完全同量級，係純浮點雜訊。已修正 decision 檔：`verdict=pass`，`overall_verdict=confirmed_pass`。此修正不影響 BL2 gate 本身（第 6 條 `-1e-5` 容忍帶原即為此而設）。
...
##### 2026-05-17 局勢更新：雙軌並進候選

原第 3、4 條因 `p18_p21_combo` 的 `p18_only` 短 scout 出現 s3=0.941（3/3 seeds，12000r），升格為 Extended Confirmation 候選。

##### 2026-05-17 Extended Confirmation 最終結果

`p18_only`（ε_start=0.04→0.01, R=1000）已完成 12000r × 6 seeds Extended Confirmation：

| 軌道 | 條件 | Extended Confirmation（12000r×6seeds）| 最終狀態 |
|---|---|---|---|
| **主軌（BL2）** | mild_cw [1.2,1.0,0.8] | L3=6/6, s3=0.940, turn=0.054 | `confirmed_pass`，唯一 BL2 anchor |
| **對照軌** | p18_only（ε_start=0.04→0.01, R=1000）| L3=5/6, mean_s3=0.735, q_std=0.046, \|γ\|≈0 | `pass_not_bl2b`：4-way AND gate 通過，但不升 BL2b |

**BL2b 升格判定**：p18_only 通過 4-way AND gate（L3=5/6 ≥ 3, \|γ\|<1e-4, entropy=1.076>1.06, q_std=0.046>0.04），但未達 BL2b 嚴格標準（L3=5/6<6/6，mean_s3=0.735<0.940）。BL2（mild_cw）維持唯一 anchor。

機制異質性備注：ε annealing 對 seeds 45/49/51 有顯著升格效應（control 下 s3=0.554/0.000/0.554→p18_only 下 0.957/0.967/0.949），但 seed=55 出現退化（control L3 s3=0.968→L1 s3=0.000）。屬高 seed 異質性機制，不具跨 seed 一致性。

**封板結論**：對照軌已完成確認，結果為 `pass_not_bl2b`；BL2（mild_cw）確定為唯一 RL anchor，研究主線不再回頭。
...
**2026-05-18 更新**：`players/rl_player.py` 於 commit `355e481`（2026-05-04）從 12D 性格模型正式切換為 9D（Enneagram 三組設計：Drivers / Stabilizers / Explorers）。此變更影響 `sample_personality()` 輸出的 key 組合與 latent signal 計算公式，導致 PERS-CAL baseline 行為漂移：

| 指標 | 12D 模型（Apr 12 鎖定） | 9D 模型（May 18 重驗） |
|------|------------------------|----------------------|
| L1 | **2/60 (3.3%)** | **5/60 (8.3%)** |
| Healthy | 51/60 (85.0%) | 45/60 (75.0%) |
| Mean s3 | 0.8743 | 0.8069 |
| gate max_l1 | 3 | **6**（更新） |
| overall_pass | PASS | PASS |

- 9D 模型為**有意識的研究決定**（已煙霧測試），lambda 參數維持不動（0.15, 0.10, 0.20, 0.20）
- gate max_l1 由 3 調整為 **6**（反映 9D 模型 L1 率略高，healthy ≥ 42 仍通過）
- locked baseline 已更新：`outputs/pers_cal_baseline_gate60_summary.json`（2026-05-18）
- `simulation/pers_cal_baseline_gate60.py` 的 `--gate-max-l1` 預設值更新為 6
...
###### Track B 啟動前置條件（P0 Checklist）

以下項目必須在 Track B 任何實驗執行前全部完成，缺一不可：

1. **`simulation/personality_rl_runtime.py::_PERSONALITY_KEYS_ORDERED` 9D 修復（靜默語意錯誤）**
   - 現狀：`_PERSONALITY_KEYS_ORDERED` 仍為 12D 舊鍵，`EventBridge.compute_reward_risk()` 對 9D personality dict 查詢時，4/9 維度靜默歸零（位置 1,2,5,8：`caution`→0, `greed`→0, `persistence`→0, `ambition`→0；`assertiveness`/`risk_aversion`/`endurance`/`curiosity` 永遠不被查詢）
   - 修復方式（單行）：將 `_PERSONALITY_KEYS_ORDERED` 改為與 `players/rl_player.py::_PERSONALITY_KEYS` 一致的 9D 順序
   - 事件模板（`02_event_templates_smoke_v1.json`）已在 `2bb4fe8` 更新為 9 weights 且語意對應 9D，**模板不需再動**
   - 狀態：✅ 已修復（2026-05-18，同步修正 `compute_reward_risk` 中的 `range(..., 12)` → `range(..., 9)`）

2. **EventBridge 9D 相容性煙霧測試**（修復後執行）
   - 以 1 seed × `events_json=02_event_templates_smoke_v1.json` × `event_rate=0.15` 確認 `compute_reward_risk()` 非零輸出且 personality 響應正常
   - 狀態：✅ 已通過（3/3 events 非零輸出，2026-05-18）
...
###### B1-H3 Delayed Feedback（Separate Spec Lock）

本節定義「假設 3：延遲 / 滯後式 world feedback」的最小契約。
此機制屬於 B1 smoke lock sheet 的**另案分支**，不得回填為原 B1 cell 結果。

1. **目的（硬鎖）**
2. 驗證即時 `adaptive_world` 是否在微觀上過度反應，導致宏觀循環被阻尼。
3. 驗證引入離散 delay 後，是否能在 lock 條件下提升 `turn_strength` / `stage3_score`。

1. **Config 欄位（最小新增）**
2. `world_feedback_delay_windows: int`（預設 `0`，單位為 world-update windows）
3. `world_feedback_smooth_windows: int`（預設 `1`，對 world update 輸入做 moving average）

1. **語意（硬鎖）**
2. 設 `u_t` 為第 `t` 個 window 的即時計算更新量（即既有 `delta_p + delta_e`）。
3. 實際套用更新量定義為：
  - `delay=0` 時：`u_t^apply = MA_m(u_t)`
  - `delay=d>0` 時：`u_t^apply = MA_m(u_{t-d})`
  - 其中 `m = world_feedback_smooth_windows`，`MA_m` 為長度 `m` 的簡單移動平均。
4. 世界狀態更新式保持不變：`s(t+1)=clamp(s(t)+lambda_world*u_t^apply,0,1)`。
5. 若歷史不足以支援 `delay` 或 `smooth`，僅使用已存在前綴窗口計算平均，不得外插。

1. **退化等價（必測）**
2. `world_feedback_delay_windows=0` 且 `world_feedback_smooth_windows=1` 時，必須 bit-exact 退化回既有 `adaptive_world` 路徑。
3. `lambda_world=0` 時，不論 delay/smooth 設定為何，世界狀態都必須維持 `(0.5,0.5,0.5,0.5)`。

1. **第一輪 H3 smoke protocol（固定）**
2. 只允許 smoke seeds：`42,44,45,67,73,90`。
3. 固定 `event_dispatch_mode=async_poisson`、`world_feedback_mode=adaptive_world`。
4. 固定 B1 lock 其餘欄位（`events_json`、fairness 規則、target rate、rounds、players）。
5. 只允許 3 個 cells：
  - `h3_ctrl`: `delay=0`, `smooth=1`
  - `h3_d1`: `delay=1`, `smooth=1`
  - `h3_d1s2`: `delay=1`, `smooth=2`
6. 第一輪不得擴增到 gate60，不得混入 subgroup / leader-policy family。

1. **輸出命名與契約（硬鎖）**
2. out-dir 前綴固定：`outputs/personality_rl_async_poisson_h3_delay_*`。
3. provenance 必填新增欄位：
  - `world_feedback_delay_windows`
  - `world_feedback_smooth_windows`
  - `world_feedback_effective_mode`（`adaptive_world_delayed` 或 `adaptive_world`）

1. **判讀規則（第一輪）**
2. 主要 smoke gate 仍只看 `fairness_fail_count=0`。
3. `turn_strength`、`mean_stage3_score`、`boundary_hit_rate` 只作 secondary 訊號；不得升級為本輪主 gate。
4. 若 H3 三個 cells 全部不優於 `h3_ctrl`，則記為 H3-first-scout negative，需先做 closure 註記再決定是否擴格。
...
1. **B2 v2（log-space multiplicative modulation）擴充契約**
2. `event_modulation_mode: "off" | "multiplicative_v2"`（預設 `off`）
3. `event_modulation_gain: float`（預設 `0.0`）
4. `event_modulation_log_center: float`（預設 `0.0`）
5. `event_modulation_zero_mean: bool`（預設 `false`）
6. `event_modulation_floor: float`、`event_modulation_ceiling: float`（預設 `1.0`）
7. 生效條件：僅在 `event_reward_mode=multiplicative` 且 `event_modulation_mode=multiplicative_v2` 啟用；其餘情況需維持既有路徑
8. `multiplicative_v2` 運算語意：
9. 先計算基礎乘數 $m_i = 1 + \delta_i$（$\delta_i$ 為事件 reward modulation），並以 $\varepsilon=10^{-6}$ 下界保護後取 $\ell_i=\log(m_i)$
10. 計算調制項 $g_i = \lambda (\ell_i - c)$，其中 $\lambda=event\_modulation\_gain$、$c=event\_modulation\_log\_center$
11. 若 `event_modulation_zero_mean=true`，則在當輪受影響玩家集合上做中心化：$g_i \leftarrow g_i - \overline{g}$
12. 新乘數為 $m_i' = \exp(\ell_i + g_i)$，再套用 `event_modulation_floor/ceiling` 夾取，最後仍需套用 `event_reward_multiplier_cap` 的 $[1-cap, 1+cap]$ 邊界
13. `event_reward_mode=additive` 或 `event_modulation_mode=off` 時，既有 B1/B2 結果必須維持 bit-exact

1. **B2 v2 診斷欄位（最小集合）**
2. Round CSV：`reward_multiplier_raw`、`reward_multiplier_clamped`、`log_reward_multiplier`、`modulation_gain_effective`、`modulation_zero_mean_residual`、`multiplicative_static_pass`
3. Seed provenance：`event_modulation_mode`、`event_modulation_gain`、`event_modulation_log_center`、`event_modulation_zero_mean`、`event_modulation_floor`、`event_modulation_ceiling`、`modulation_zero_mean_residual_max`、`multiplicative_static_pass`
...
1. **B3 v2 擴充相容欄位（kernel profile）**
2. `impact_spread_kernel_id: str`（預設 `legacy_v1`，允許 `legacy_v1 | hierarchical_v2`）
3. `impact_spread_local_mass: float`、`impact_spread_neighbor_mass: float`（僅 `spread` 模式生效；執行時需做非負化與正規化，確保 local+neighbor=1）
4. `impact_spread_neighbor_hop: int`（最小 `0`，`0` 代表僅 local）
5. `impact_spread_memory_kernel: int`（最小 `1`，用於對齊 B3 v2 CLI 契約）
...
5. `impact_spread_kernel_id`
6. `impact_kernel_mass_local`
7. `impact_kernel_mass_neighbor`
8. `impact_spread_radius`
9. `impact_spread_delay_mean`
10. `impact_spread_alignment`
11. `impact_kernel_mass_error`
12. `impact_spread_applied`
...
## 11) 理論驗證階段封板快照（2026-05-18）

> **完整階段收官報告**：`Phase_Closure_Report_20260518.md`

本節為 2026-05-18 理論與模擬驗證階段的正式封板快照，依據完整報告摘錄關鍵結論與數值底牌，作為後續 Runtime Bridge 階段的可追溯入口。

### 11.1 三大管線封板結論

| 管線 | 最終判決 | 主要產物 |
|---|---|---|
| B-series B1~B5（離散同步複製子） | ✅ 全數 `close`，無 Longer Confirm 候選 | `研發日誌.md` §B-series 各 closure 節 |
| E1 RL 主軌 BL2（mild_cw [1.2,1.0,0.8]） | ✅ `confirmed_pass`，唯一 Runtime Bridge Anchor | `outputs/p21a_extended/p21a_decision.md` |
| E1 RL 對照軌 p18_only（ε-annealing） | ✅ `pass_not_bl2b`，對照軌正式關閉 | `outputs/p18_only_extended/` |
| Track A A3 Protocol Regression（9D, 60-seed） | ✅ `overall_pass=True`，40 passed | `outputs/track_a_protocol_regression_summary.json` |

### 11.2 BL2 數值底牌（Runtime Bridge Anchor）

| 指標 | 值 | Gate 閾值 | 判定 |
|---|---|---|---|
| L3 seeds | **6/6 (100%)** | ≥ 6/6 | ✅ |
| mean_s3 | **0.940** | ≥ 0.940 | ✅ |
| \|env_gamma\| | **< 1e-10** | < 1e-4 | ✅ |
| q_std | **0.04567** | > 0.040 | ✅ |
| mean_player_weight_entropy | **1.077** | ≥ 1.06 | ✅ |

鎖定參數（不得在 Runtime Bridge 階段單方面更動）：
- `mild_cw = [1.2, 1.0, 0.8]`
- `seeds = [45, 47, 49, 51, 53, 55]`，`rounds = 12000`，`burn-in = 4000`，`tail = 4000`
- CC1 fallback: `r2=0.85`，`min_rotation=20.0`

### 11.3 Track A 9D Baseline 鎖定值（2026-05-18 更新）

| 指標 | 12D baseline（Apr 12，已廢棄） | 9D baseline（現行，355e481） |
|---|---|---|
| L1 / 60 seeds | 2/60 (3.3%) | **5/60 (8.3%)** |
| healthy / 60 seeds | 51/60 | **45/60** |
| mean_s3 | 0.8743 | **0.8069** |
| gate.max_l1 | 3 | **6**（品質紅線 ≤10%）|
| overall_pass | True | **True** |

鎖定檔：`outputs/pers_cal_baseline_gate60_summary.json`

### 11.4 技術債記錄（延期至 Track B）

~~`simulation/personality_rl_runtime.py` 的 `_PERSONALITY_KEYS_ORDERED` 殘留 12D 舊鍵（12 個），與現行 9D Enneagram 三組模型（9 個）不一致。此不一致處於 **Dead Zone**（PERS-CAL baseline 不呼叫 EventBridge，A1/A2/A3 gate 完全不受影響）。Track B 重啟時需一並修正。~~
✅ 已修復（2026-05-18）：`_PERSONALITY_KEYS_ORDERED` 已更新為 9D Enneagram 三組 9 鍵，`compute_reward_risk` 的 `range(..., 12)` 同步改為 `range(..., 9)`，詳見 §11.3 Track B Checklist 項目 1。

### 11.5 下一階段進入條件（Runtime Bridge）

以下項目均已滿足，可立即啟動 Runtime Bridge：

- [x] BL2 anchor 唯一性確認（p18_only 對照軌關閉）
- [x] 9D baseline 封板（max_l1=6，overall_pass=True）
- [x] A2 schema contract 完整（40 pytest passed）
- [x] EventBridge dead zone 隔離標記完成
- [x] B-series 全線關閉（B1~B5 均 `close`）

---

## 12) Runtime Bridge v2：Godot 前端整合架構規格（2026-05-18 起稿）

本節定義將已驗證的 BL2 RL 循環機制接入 Godot 前端的最小工程規格。設計遵循 SDD 分層不變條件：`analysis/` 不依賴 `simulation/`，`evolution/` 保持純函式，`api/` 負責傳輸層與 session 管理。

### 12.1 分層與職責劃分（新增 Runtime Bridge 層）

```
Godot Frontend（UI rendering）
      │ HTTP/WebSocket
      ▼
api/server.py
      │ (additive, 不破壞既有 GameEngine 路徑)
      ├─ GET|POST /sessions/{id}/reset
      ├─ POST /sessions/{id}/step
      ├─ GET  /sessions/{id}/snapshot
      │
      ▼
api/rl_session_manager.py  [新增]
      │ (session pool, lifecycle management)
      │
      ├─ init_rl_session(config)
      ├─ poll_session(session_id)
      └─ step_session(session_id, action_hints)
      │
      ▼
simulation/rl_session_engine.py  [新增]
      │ (batch → step-driven 轉換)
      │
      ├─ RLSessionEngine.reset()  → 初始化 + burn-in 隊列
      ├─ RLSessionEngine.step()   → 推進 1 round + 更新 state buffer
      └─ RLSessionEngine.snapshot() → 產生 FrameSnapshot（無推進）
      │
      ├─ evolution/independent_rl.py  [pure functions, BL2 locked]
      │  ├─ rl_q_update()
      │  ├─ boltzmann_select()
      │  └─ strategy_payoff_matrix()
      │
      ├─ players/rl_player.py  [9D personality mapping, Track A locked]
      │  └─ RLPlayer dataclass
      │
      └─ analysis/cycle_metrics.py  [pure function snapshot, read-only]
         ├─ detect_cycle_level()
         └─ compute_stage_scores()
```

**分層不變條件（hard constraints）**：
1. `analysis/` 只呼叫 `cycle_metrics.py` 的 pure functions，不直接 import `simulation/`
2. `evolution/` 只實作算子，不做 I/O、不維持全局狀態
3. `api/` 是唯一的傳輸層，負責 JSON serialization 與 session lifecycle
4. `simulation/` 的職責是 I/O 協調與 episode batching（對舊路徑）+ step-driven wrapping（新 RL 路徑）

### 12.2 核心資料結構：RLSessionEngine 介面簽章

```python
@dataclass
class RLSessionConfig:
    """Runtime Bridge 配置（subset of PersonalityRLConfig，鎖定 BL2 參數）"""
    ...
    strategy_alpha_multipliers: list[float] = [1.2, 1.0, 0.8]  # mild_cw [c1, c2, c3]
    a: float = 1.0
    b: float = 0.9
    cross: float = 0.20
    ...
    burn_in: int = 4000
    tail: int = 4000
    check_interval: int = 200  # cycle_level check frequency (rounds)
    
    # ---- Personality modulation (not locked) ----
    personality_mode: str = "none"  # "none" | "random_9persona"
    seed: int = 42
```

### 12.3 FrameSnapshot 結構（ResponseEnvelope 擴充）
...
```json
{
  ...
  "extensions": {
    ...
    "rl_q_std": 0.0467,
    "rl_entropy": 1.074,
    "rl_env_gamma": -5.3e-11,
    
    "rl_p_aggressive": 0.38,
    "rl_p_defensive": 0.31,
    "rl_p_balanced": 0.31,
    
    "rl_pi_aggressive": 0.35,
    "rl_pi_defensive": 0.35,
    "rl_pi_balanced": 0.30,
    
    "rl_q_mean_aggressive": 0.135,
    "rl_q_mean_defensive": 0.142,
    "rl_q_mean_balanced": 0.138,
    
    "rl_personality_mode": "none",
    "rl_round": 4512,
    "rl_warm": true,
    "rl_phase": "tail"
  }
...
**欄位語意**：
- `rl_cycle_level`：Stage1(0) / Stage2(1) / Stage3(2)（three-stage classifier）
- `rl_s3_score`：Stage3 quality score，範圍 [0, 1]
- `rl_p_*`：realized action proportions（由上一輪抽樣選擇）
- `rl_pi_*`：mean policy probabilities（由 Boltzmann weights + alpha/beta）
- `rl_q_mean_*`：per-strategy mean Q values（累積選擇權重）
- `rl_warm`：burn-in 完成標誌（false → 預熱中，front-end 應停用 UI；true → 可開始互動）
- `rl_phase`："burn-in" | "tail"（當前窗口位置，用於 cycle metrics context）

### 12.4 Session 生命週期與非同步 burn-in

```
┌─ POST /sessions/initialize ───────────────────────────────────┐
│  Input: { personality_mode, seed }                            │
│  Output: { session_id, status="burn-in", tick=0, warm=false } │
│  ├─ 創建 RLSessionEngine(config)                              │
│  ├─ 啟動非同步 burn-in loop（4000 rounds）                    │
│  │  └─ 後台執行，不阻塞 HTTP 回應                             │
│  └─ 返回 session_id（front-end 開始 poll）                    │
│                                                                │
│  ┌─ Background: burn-in loop ──────────────────────────────┐  │
...
┌─ POST /sessions/{id}/step ────────────────────────────────────┐
│  Input: { action_hints?: dict } (optional, 未來拓展)          │
│  Output: { tick, rl_cycle_level, rl_s3_score, ... }          │
│  (等待 warm=true 後才真正推進，burn-in 期間返回佔位值)        │
│  ├─ if not warm: return placeholder snapshot (tick=4000)      │
│  ├─ else: step() → rl state update + cycle check             │
│  └─ return FrameSnapshot                                      │
│                                                                │
│  [會被 Godot 每幀呼叫，推進 RL engine]                       │
└────────────────────────────────────────────────────────────────┘

┌─ GET /sessions/{id}/snapshot ─────────────────────────────────┐
│  Input: none                                                   │
│  Output: { tick, rl_cycle_level, ... } (current state)        │
│  ├─ 不推進 tick                                               │
│  └─ 返回 RL engine 當前快照                                   │
│                                                                │
│  [用於診斷、UI 重新整理等非推進場景]                         │
...
│  Input: none                                                   │
│  Output: { session_id, status="burn-in", warm=false }         │
│  ├─ 重置 RL engine 內部狀態                                   │
│  ├─ 清空 round buffers                                        │
│  ├─ 重新啟動非同步 burn-in                                    │
│  └─ 保留 session_id（Godot 無需重新 init）                    │
└────────────────────────────────────────────────────────────────┘
```

### 12.5 Cycle Metrics 即時檢測（sliding-window 輕量版）

RLSessionEngine 內部維護 `tail_buffer = deque(maxlen=4000)`，存儲最近 4000 rounds 的 `(p_agg, p_def, p_bal)` 三元組。

每 `check_interval=200` rounds 計算一次：

```python
def _check_cycle_metrics(engine):
    """每 200 rounds 檢查一次 cycle 狀態。"""
    if len(engine.tail_buffer) < 200:
        return None  # 未足夠樣本
    
    # 轉換為 np array，shape=(buffer_len, 3)
    buffer_array = np.array(list(engine.tail_buffer))
    
    # 呼叫 pure function（analysis/cycle_metrics.py）
    result = detect_cycle_level(
        p_trajectory=buffer_array,
        burn_in_window=4000,
        tail_window=4000
    )
    
    return {
        "cycle_level": result["stage"],
        "s3_score": result["s3"],
        "env_gamma": result["gamma"],
...
**不違反 SDD 分層**：
- 呼叫的是 `analysis/cycle_metrics.py` 中的 pure function
- 不做反向依賴（`analysis/` 仍不 import `simulation/`）
- RLSessionEngine 只維護 local buffer，不暴露給 `analysis/`

### 12.6 輸出契約（向後相容性）

新增的 RL runtime 不破壞既有 CSV 路徑（batch mode 仍用舊 `simulation/run_simulation.py`）。

**Round-level metrics 新欄位**（若選擇同時 dump 到 CSV）：
- `rl_cycle_level`, `rl_s3_score`, `rl_env_gamma`, `rl_entropy`, `rl_q_std`（來自 cycle_metrics）
- `rl_p_aggressive`, `rl_p_defensive`, `rl_p_balanced`（realized proportions）
- `rl_pi_aggressive`, `rl_pi_defensive`, `rl_pi_balanced`（policy probabilities）
- `rl_q_mean_aggressive`, `rl_q_mean_defensive`, `rl_q_mean_balanced`（Q value means）

**向後相容**：舊的 `w_*` 欄位在 batch 模式中繼續使用；step-driven 模式不使用 `w_*`（改用 `rl_pi_*` 與 `rl_q_*`）。

### 12.7 實作順序與驗收標準

#### Phase 1：核心 Engine 架構（完成 item 2, 6）

| Item | 檔案 | 責任 | 驗收標準 |
|---|---|---|---|
| 2.1 | `simulation/rl_session_engine.py` | RLSessionEngine class 框架 | `reset()` 返回初始狀態，`step()` 推進 1 round，`snapshot()` 無推進取狀態 |
| 2.2 | `simulation/rl_session_engine.py` | burn-in 隊列與 tail buffer | 初始化後 `warm=false`；after 4000 rounds `warm=true` |
| 2.3 | `simulation/rl_session_engine.py` | cycle metrics polling | 每 200 rounds 檢查一次，返回 cycle_level/s3_score |
| 6.1 | `tests/test_rl_session_engine.py` | 單輪循環測試 | step() 後 tick 遞增，buffer 更新，seed 45~55 全通過 |
| 6.2 | `tests/test_rl_session_engine.py` | burn-in 完成測試 | 4000 rounds 後 warm=true |
| 6.3 | `tests/test_rl_session_engine.py` | reset 測試 | 重置後 tick=0，warm=false，buffer 清空 |

#### Phase 2：Session 管理與 API 層（完成 item 3, 4, 5）

| Item | 檔案 | 責任 | 驗收標準 |
|---|---|---|---|
| 3.1 | `api/rl_session_manager.py` | session pool 初始化 | 支援 max_sessions=100，FIFO eviction |
| 3.2 | `api/rl_session_manager.py` | 生命週期管理 | init/poll/step/reset 端點對應 |
| 4.1 | `api/schemas.py` | ResponseEnvelope.extensions 擴充 | 新增 rl_* 欄位定義，不刪除既有欄位 |
| 4.2 | `api/schemas.py` | FrameSnapshot 轉換函式 | `rl_state_to_envelope()` 將 RLSessionEngine 狀態映射到 JSON |
| 5.1 | `api/server.py` | 新增 /sessions/{id}/reset 端點 | POST 200 OK，返回 ResponseEnvelope |
| 5.2 | `api/server.py` | 新增 /sessions/{id}/step 端點 | POST 200 OK，tick 遞增，返回 FrameSnapshot |
| 5.3 | `api/server.py` | 新增 /sessions/{id}/snapshot 端點 | GET 200 OK，不推進 tick |
| 5.4 | `api/server.py` | 新增 /sessions/initialize 端點 | POST 201 Created，返回 session_id，warm=false |

#### Phase 3：端到端驗收（完成 item 7）

| Item | 驗收條件 | 成功標誌 |
|---|---|---|
| 7.1 | reset → 初始化 + 非同步 burn-in 啟動 | session_id 返回，warm=false |
| 7.2 | poll snapshot（burn-in 期間） | 返回佔位值（cycle_level=0，s3=0） |
| 7.3 | wait 10s（burn-in 完成） | warm=true，可開始 step |
| 7.4 | step × 1000（tail 窗口） | cycle_level ∈ {1,2,3}，s3 > 0.5 |
| 7.5 | reset | tick 歸零，warm=false，可重複 cycle |
| 7.6 | 6-seed stress test（seeds 45~55） | 均通過 cycle detection，無 crash |

### 12.8 不得做的事（禁止區）

1. **不得在此階段修改 BL2 參數**（mild_cw, alpha, beta 等）  
   - 所有參數來自 §11.2 鎖定值，不得在 Runtime Bridge spec 中突破

2. **不得修改 `evolution/independent_rl.py`**  
   - 保持純函式，不做 I/O 或全局狀態

3. **不得修改 `players/rl_player.py` 的 9D 映射**  
   - Track A 已封板，不破壞 A3 protocol regression

4. **不得在 Phase 1~2 啟動 EventBridge**  
   - Dead Zone 延期至 Track B，phase 1~2 限制 `events_json=""`

5. **不得破壞 ResponseEnvelope v1 contract**  
   - 只做 additive change（新增 extensions.*），不刪改既有欄位

### 12.9 技術債與 Track B 待辦

- [x] EventBridge 12D→9D 金鑰一致性修復（`_PERSONALITY_KEYS_ORDERED`）（已修復 2026-05-18，見 §11.4）
- [ ] step-driven 模式下的 CSV dump 路徑設計
- [ ] Godot frontend 與 `/sessions/initialize` 的整合示例

---

*本規格遵循 SDD 分層不變條件，基於 §11 的 BL2 anchor 與 Track A baseline，為下一階段實作提供可追溯的形式化定義。所有實作均需先通過此規格的驗收標準，才得進入下一 phase。*
